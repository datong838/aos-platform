...
- 已在内置浏览器重新打开 `/aip/capacity`，逐项点击“用量仪表盘”“速率限制”“预留容量”，并核对完整 AIP 侧栏、页签切换和深链。
- `p6c-usage-month-final.png`：近 30 天回读 16 条 Usage Receipt，其中实测 16 条；模型用量为 2.9K、精确值 2,928 个 Token，并明确披露业务时区 `Asia/Shanghai` 以及周/月为滚动 7/30 天。今日与近 7 天无 Receipt 时保持“未观测”，没有缺证归零。
- “速率限制”回读 3 个 exact CapacityPool，均显示每次租约 2.0K 个模型用量单位、并发 0/2；当前没有单独用户限额时明确继承项目默认限额。
- “预留容量”回读相同 3 个 canonical pool；模型路由安全处置入口已实点并进入 `/aip/model-router`，未调用 Provider、未切换生产流量。
