...
describe("aipAgentControl read-only exact-ref client", () => {
  ...
  it("按实例读取最近 AgentRun 且限制查询参数", async () => {
    const run = { tenant, agentRunId: "run-1", taskId: "task-1", taskRunId: "task-run-1", instanceId: "agent-1", instanceVersion: 1, skillBindingId: "binding-1", request: { taskRef: resource("Task", "task-1"), planRef: resource("PlanRevision", "plan-1"), agentInstance: asset("AgentInstance", "agent-1"), skill: asset("SkillTemplate", "skill-1"), logic: asset("LogicRevision", "logic-1"), modelRoute: asset("ModelRouteRevision", "route-1"), policy: asset("RuntimePolicyRevision", "policy-1"), inputRefs: [] }, status: "succeeded", version: 1, createdAt: "2026-08-30T10:00:00Z", updatedAt: "2026-08-30T10:01:00Z" };
    apiGet.mockResolvedValue({ tenant, items: [run], count: 1 });
    await expect(aipAgentControl.listAgentRuns("agent-1", 3)).resolves.toMatchObject({ count: 1 });
    expect(apiGet).toHaveBeenCalledWith("/v1/aip/agent-runs?limit=3&instance_id=agent-1");
    await expect(aipAgentControl.listAgentRuns("../agent", 3)).rejects.toThrow("instanceId 非法");
    await expect(aipAgentControl.listAgentRuns("agent-1", 0)).rejects.toThrow("limit 非法");
  });
