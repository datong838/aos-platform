...
function tenantEcho(value: unknown, expected?: Tenant): Tenant { const parsed=tenant(value); if(expected&&(parsed.orgId!==expected.orgId||parsed.projectId!==expected.projectId)) throw new Error("tenant echo 不一致"); return parsed; }
...
export function parseMediaJobSnapshot(value: unknown, expectedTenant?: Tenant): MediaJobSnapshot {
