...
describe("W2-C production contract SDK",()=>{
  ...
  it("编译只 POST canonical Plan 请求并携带幂等键",async()=>{
    ...
    transport.apiPost.mockResolvedValue({tenant,taskId:"task-1",templateRef:exact("StageTemplateRevision","stage-1"),responsibilityPlanRef:input.responsibilityPlanRef,productionContextRef:input.productionContextRef,planRef:exact("PlanRevision","plan-revision-1"),compilerVersion:"w2c.v1",inputHash:hash,compilationHash:hash,normalizedStageIds:["analysis"],applicableStageIds:["analysis"],notApplicableStageIds:[],createdAt:"2026-08-14T00:00:00Z"});
