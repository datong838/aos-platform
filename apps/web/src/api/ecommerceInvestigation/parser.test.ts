...
const viewV4 = { ...view, schemaVersion: "aos.ecommerce.business-investigation-workbench-view/v4", artifacts: ["BusinessDossierRevision", "ProblemMapRevision", "OpportunityMapRevision", "SolutionPortfolioRevision"].map((artifactType) => ({ artifactType, status: "missing", artifactRef: null, bindingId: null, bindingHash: null, selectionRevision: null, dataCutoff: null, lineageRef: null })), evidence: { status: "exact", exactRefs: [ref("EvidenceBundleRevision", "evidence-1")], locatorRefs: [resourceRef("EvidenceBundleRevision", "evidence-1")] }, timeline: [
  { eventId: "case:case-1:r1", eventType: "case_revision", title: "Case revision 建立", occurredAt: "2026-08-26T08:00:00Z", exactRef: ref("BusinessInvestigationCaseRevision", "case-1"), relatedRef: null },
...
describe("ecommerceInvestigation strict parser", () => {
  ...
  it("解析 v4 Evidence/Artifact/Timeline 并保留 v3 诚实兼容", () => {
    const parsed = parseInvestigationWorkbenchView(viewV4, "run-1", tenant);
    expect(parsed.drilldownVersion).toBe("canonical-v4");
    expect(parsed.evidence.exactRefs[0]?.resourceId).toBe("evidence-1");
