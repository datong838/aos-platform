...
const assignmentObservation = { tenant: cockpitBase.tenant, runRef: { resourceType: "TaskRun", resourceId: "run:1", version: 4 }, takeoverRequests: [], takeoverDecisions: [], assignmentLeases: [], evaluatedAt: cockpitBase.evaluatedAt };
const handoffCompile = { schemaVersion: "aos.ecommerce-workshop.module-handoff-compile/v1", tenant: cockpitBase.tenant, runId: "run:1", taskId: "task-1", evaluatedAt: cockpitBase.evaluatedAt, responsibilityPlanRef: productionContext.responsibilityPlanRef, sourceModuleId: "ecommerce.task-cockpit", targetModuleId: "ecommerce.operations", sourceSlotId: "collector", targetSlotId: "reviewer", readiness: "blocked", blockers: [{ code: "INSTANCE_READINESS_UNVERIFIED", dependency: "agent-review", requiredAction: "refresh exact readiness" }], issueCommand: null, sideEffects: { handoffsIssued: 0, tokensMinted: 0, decisionsCreated: 0, agentRunsStarted: 0 } };
...
const skillContributions = { schemaVersion: cockpitBase.schemaVersion, tenant: cockpitBase.tenant, runId: "run:1", taskId: "task-1", evaluatedAt: cockpitBase.evaluatedAt, projectionStatus: "blocked", blockerCodes: ["NO_CANONICAL_AGENT_RUN_CONTRIBUTION"], items: [] };
...
const sourceReadiness = { schemaVersion: "aos.source-readiness/v1", tenant: { orgId: "org-org", projectId: "dev-project" }, checkedAt: sourceCheckedAt, cutoffAt: sourceCheckedAt, status: "blocked", receiptRef: null, investigation: { requirementRef: { resourceType: "DataRequirementRevision", resourceId: "requirement-1", revision: "2", contentHash: "b".repeat(64), authority: "Data" }, status: "blocked", evaluatedAt: sourceCheckedAt, requiredCutoff: sourceCheckedAt, freshnessExpiresAt: null, requiredFactCount: 2, coveredFactCount: 1, coverageRatio: 0.5, unmetFacts: ["repeat_purchase"], blockers: [{ code: "FACT_COVERAGE_PARTIAL", fact: "repeat_purchase", sourceIds: ["niushop-qyh"], reason: "当前事实覆盖不足" }] }, sources: sourcePipelines.map((pipelineId) => ({ schemaVersion: "aos.source-readiness/v1", tenant: { orgId: "org-org", projectId: "dev-project" }, sourceId: "niushop-qyh", pipelineId, objectType: "Object", status: "blocked", checkedAt: sourceCheckedAt, observedAt: sourceCheckedAt, sourceEventAt: null, projectedAt: sourceCheckedAt, dataCutoff: sourceCheckedAt, freshnessExpiresAt: null, sourceConfigRef: null, mappingRef: null, schemaRef: null, maskingPolicyRef: null, freshnessPolicyRef: null, qualityPolicyRef: null, reconciliationPolicyRef: null, queryCapabilityRef: null, latestRun: { runId: "run-1", status: "succeeded", scheduledFor: sourceCheckedAt, startedAt: sourceCheckedAt, finishedAt: sourceCheckedAt, rowsWritten: 1, errorCode: null }, counts: { sourceTotal: 1, sourceActive: 1, sourceDeleted: 0, projectionTotal: 1, unexplainedDelta: 0 }, quality: { status: "unknown", ruleRef: null, summary: null }, reconciliation: { status: "unknown", ruleRef: null, summary: null }, reasons: sourceBlockers, blockers: sourceBlockers })) };
...
const commandInput = { commandId: "classify" as const, request: { governance: { proposalId: "proposal-1" }, expectedVersion: 0 } };
const commandPreview = { schemaVersion: "aos.ecommerce-workshop.operation-command-preview/v1", tenant: envelope.tenant, commandId: "classify", actionTypeId: "ecommerce.operation.classify", previewHash: "a".repeat(64), proposalId: "proposal-1", proposalHash: "b".repeat(64), leaseId: "lease-1", sideEffect: "internalAuthority", externalEffectAllowed: false, confirmPath: "/v1/ecommerce-workshop/commands/operations/classify", evaluatedAt: "2026-08-24T08:00:00Z" };
const commandExecution = { schemaVersion: "aos.ecommerce-workshop.operation-command-execution/v1", tenant: envelope.tenant, commandId: "classify", status: "applied", proposalId: "proposal-1", leaseId: "lease-1", operationReceipt: { tenant: envelope.tenant, receiptId: "operation-receipt-1", operation: "operation_classification.append", idempotencyKey: "command-key-1", requestHash: "c".repeat(64), resultRef: { resourceId: "classification-1", revision: 1, contentHash: "d".repeat(64) }, createdBy: "user:executor", createdAt: "2026-08-24T08:00:01Z" } };
...
describe("EcommerceWorkshopClient", () => {
  ...
  it("只发 canonical SourceReadiness GET 并保留 blocked", async () => {
    ...
    expect(result.investigation).toMatchObject({ status: "blocked", requirementRef: { resourceId: "requirement-1" } });
  ...
  it("先预览再携同一 hash 确认，并且没有 refund 执行路径", async () => {
    const receiptObservation = { ...commandObservation, proposalId: "proposal-1", leaseId: "lease-1" };
    const fetch = vi.fn().mockResolvedValueOnce(ok(commandPreview)).mockResolvedValueOnce(ok(commandExecution)).mockResolvedValueOnce(ok(receiptObservation));
    const client = new EcommerceWorkshopClient({ fetch, getBaseUrl: () => "http://api.test", getAuthHeaders: () => ({ Authorization: "Bearer test" }) });
    const preview = await client.previewOperationCommand(commandInput);
    await expect(client.confirmAndReadOperationCommand(commandInput, preview.previewHash, "command-key-1")).resolves.toMatchObject({ execution: { status: "applied", operationReceipt: { receiptId: "operation-receipt-1" } }, observation: { status: "applied", operationReceiptId: "operation-receipt-1" } });
    expect(fetch).toHaveBeenNthCalledWith(1, "http://api.test/v1/ecommerce-workshop/commands/operations/previews", expect.objectContaining({ method: "POST", body: JSON.stringify(commandInput) }));
    expect(fetch).toHaveBeenNthCalledWith(2, "http://api.test/v1/ecommerce-workshop/commands/operations/classify", expect.objectContaining({ method: "POST", body: JSON.stringify(commandInput.request), headers: expect.objectContaining({ "X-Workshop-Preview-Hash": preview.previewHash, "Idempotency-Key": "command-key-1" }) }));
    expect(fetch).toHaveBeenNthCalledWith(3, "http://api.test/v1/ecommerce-workshop/commands/operations/observations/proposal-1/leases/lease-1", expect.objectContaining({ method: "GET" }));
    await expect(client.confirmOperationCommand({ commandId: "refund", request: {} } as never, "a".repeat(64), "key")).rejects.toBeInstanceOf(TypeError);
  });
  ...
  it("只发八条 Task Cockpit canonical GET 并规范编码 query", async () => {
    const fetch = vi.fn().mockResolvedValueOnce(ok(core)).mockResolvedValueOnce(ok(details)).mockResolvedValueOnce(ok(details)).mockResolvedValueOnce(ok(productionContext)).mockResolvedValueOnce(ok(responsibilityHandoffs)).mockResolvedValueOnce(ok(approvalReview)).mockResolvedValueOnce(ok(actionReceipts)).mockResolvedValueOnce(ok(skillContributions));
    const client = new EcommerceWorkshopClient({ fetch, getBaseUrl: () => "http://api.test/", getAuthHeaders: () => ({ Authorization: "Bearer test" }) });
    ...
    await expect(client.getTaskCockpitRunSkillContributions("run:1")).resolves.toMatchObject({ projectionStatus: "blocked", blockerCodes: ["NO_CANONICAL_AGENT_RUN_CONTRIBUTION"] });
    ...
    expect(fetch).toHaveBeenNthCalledWith(8, "http://api.test/v1/ecommerce-workshop/views/task-cockpit/runs/run%3A1/skill-contributions", expect.objectContaining({ method: "GET" }));
  });
  it("只向 run-scoped compiler 发零副作用 POST，并严格回读 blocked", async () => {
    const fetch = vi.fn().mockResolvedValue(ok(handoffCompile));
    const client = new EcommerceWorkshopClient({ fetch, getBaseUrl: () => "http://api.test", getAuthHeaders: () => ({ Authorization: "Bearer test" }) });
    const input = { handoffId: "handoff-2", taskRef: { resourceType: "Task", resourceId: "task-1", revision: "1", authority: "aip-task-runtime" }, runRef: { resourceType: "TaskRun", resourceId: "run:1", revision: "1", authority: "aip-task-runtime" }, sourceModuleId: "ecommerce.task-cockpit", targetModuleId: "ecommerce.operations", sourceSlotId: "collector", targetSlotId: "reviewer", purpose: "review", requestedOutcome: "accepted decision", objectRefs: [], artifactRefs: [], evidenceRefs: [], context: {}, allowedContextFields: [], markings: ["internal"], expiresAt: "2026-08-15T11:00:00Z", correlationRef: null };
    await expect(client.compileTaskCockpitRunHandoff("run:1", input)).resolves.toMatchObject({ readiness: "blocked", issueCommand: null });
    expect(fetch).toHaveBeenCalledWith("http://api.test/v1/ecommerce-workshop/views/task-cockpit/runs/run%3A1/handoffs/compile", expect.objectContaining({ method: "POST", headers: expect.objectContaining({ Authorization: "Bearer test", "Content-Type": "application/json" }), body: JSON.stringify(input) }));
    await expect(client.compileTaskCockpitRunHandoff("bad/run", input)).rejects.toBeInstanceOf(TypeError);
  });
  it("只读 exact Run 的职责接管 authority，不携带 tenant 查询参数", async () => {
    const fetch = vi.fn().mockResolvedValue(ok(assignmentObservation));
    const client = new EcommerceWorkshopClient({ fetch, getBaseUrl: () => "http://api.test", getAuthHeaders: () => ({ Authorization: "Bearer test" }) });
    await expect(client.getResponsibilityAssignmentObservation("run:1")).resolves.toMatchObject({ runRef: { resourceId: "run:1", version: 4 }, assignmentLeases: [] });
    expect(fetch).toHaveBeenCalledWith("http://api.test/v1/aip/responsibility-assignments/runs/run%3A1/observation", expect.objectContaining({ method: "GET", headers: expect.objectContaining({ Authorization: "Bearer test" }) }));
    await expect(client.getResponsibilityAssignmentObservation("bad/run")).rejects.toBeInstanceOf(TypeError);
  });
