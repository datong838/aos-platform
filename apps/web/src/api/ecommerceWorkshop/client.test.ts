...
describe("EcommerceWorkshopClient", () => {
  ...
  it("只发六条 Task Cockpit canonical GET 并规范编码 query", async () => {
    const fetch = vi.fn().mockResolvedValueOnce(ok(core)).mockResolvedValueOnce(ok(details)).mockResolvedValueOnce(ok(details)).mockResolvedValueOnce(ok(productionContext)).mockResolvedValueOnce(ok(responsibilityHandoffs)).mockResolvedValueOnce(ok(approvalReview));
    const client = new EcommerceWorkshopClient({ fetch, getBaseUrl: () => "http://api.test/", getAuthHeaders: () => ({ Authorization: "Bearer test" }) });
