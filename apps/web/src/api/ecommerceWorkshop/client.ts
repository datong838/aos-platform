...
import type { AnalystViewResponse, ContentCampaignViewResponse, CreatorGrowthViewResponse, CustomerViewResponse, EcommerceWorkshopApiErrorBody, EcommerceWorkshopModuleListResponse, EcommerceWorkshopModuleReadinessResponse, MediaStudioViewResponse, OperationCommandObservationResponse, OperationCommandReadinessResponse, OperationsViewResponse, PriceGovernanceViewResponse, SourceReadinessEnvelope, TaskCockpitActionReceiptResponse, TaskCockpitApprovalReviewResponse, TaskCockpitCheckpointPageResponse, TaskCockpitCoreQuery, TaskCockpitCoreResponse, TaskCockpitProductionContextResponse, TaskCockpitResponsibilityHandoffResponse, TaskCockpitRunDetailQuery, TaskCockpitStepPageResponse, WorkshopSharedContextResponse } from "./contracts";
import { parseAnalystView, parseContentCampaignView, parseCreatorGrowthView, parseCustomerView, parseEcommerceWorkshopApiError, parseEcommerceWorkshopModuleList, parseEcommerceWorkshopModuleReadiness, parseMediaStudioView, parseOperationCommandObservation, parseOperationCommandReadiness, parseOperationsView, parsePriceGovernanceView, parseSourceReadinessEnvelope, parseTaskCockpitActionReceipts, parseTaskCockpitApprovalReview, parseTaskCockpitCheckpoints, parseTaskCockpitCore, parseTaskCockpitProductionContext, parseTaskCockpitResponsibilityHandoffs, parseTaskCockpitSteps, parseWorkshopSharedContext } from "./parser";
...
export class EcommerceWorkshopClient {
  ...
  async getMediaStudioView(): Promise<MediaStudioViewResponse> { return parseMediaStudioView(await this.get("ecommerceWorkshopMediaStudioViewGet", "/v1/ecommerce-workshop/views/media-studio")); }
  async getAnalystView(): Promise<AnalystViewResponse> { return parseAnalystView(await this.get("ecommerceWorkshopAnalystViewGet", "/v1/ecommerce-workshop/views/analyst")); }
  async getPriceGovernanceView(): Promise<PriceGovernanceViewResponse> { return parsePriceGovernanceView(await this.get("ecommerceWorkshopPriceGovernanceViewGet", "/v1/ecommerce-workshop/views/price-governance")); }
  async getCustomerView(): Promise<CustomerViewResponse> { return parseCustomerView(await this.get("ecommerceWorkshopCustomerViewGet", "/v1/ecommerce-workshop/views/customer")); }
  async getSharedContext(contextId: string): Promise<WorkshopSharedContextResponse> { if (!SHARED_CONTEXT_ID.test(contextId)) throw new TypeError("contextId 无效"); return parseWorkshopSharedContext(await this.get("ecommerceWorkshopSharedContextGet", `/v1/ecommerce-workshop/contexts/${encodeURIComponent(contextId)}`), contextId); }
