import {
  ...
  CREATOR_PREPARE_SCHEMA_VERSION,
  CREATOR_LIFECYCLE_SCHEMA_VERSION,
  type CreatorActionKind,
  type CreatorActionLaneBinding,
  type CreatorBatchPreparation,
  type CreatorBatchStartDecision,
  type CreatorContractRevision,
  type CreatorContributionViewResponse,
  type CreatorDeliveryObservation,
  type CreatorLifecycleViewResponse,
  type CreatorPrepareBatchItem,
  type CreatorPrepareExactRef,
  type CreatorRelationshipRevision,
  type CreatorStartLedger,
...
function parseTaskCockpitReviewIssueEvent(value: unknown): TaskCockpitReviewIssueEvent {
  const raw = record(value, "taskCockpit.reviewIssue.event"); exact(raw, ["eventId", "sequence", "eventType", "issueVersion", "payloadHash", "payload", "payloadReadiness", "actor", "createdAt"], "taskCockpit.reviewIssue.event"); const payloadHash = boundedText(raw.payloadHash, "taskCockpit.reviewIssue.event.payloadHash", 64); if (!RAW_SHA256.test(payloadHash)) throw new TypeError("taskCockpit.reviewIssue event hash 漂移");
  const payload = nullable(raw.payload, (item) => record(item, "taskCockpit.reviewIssue.event.payload")); const payloadReadiness = enumValue(raw.payloadReadiness, ["exact", "legacy_unavailable"] as const, "taskCockpit.reviewIssue.event.payloadReadiness"); if ((payloadReadiness === "exact") !== Boolean(payload)) throw new TypeError("taskCockpit.reviewIssue event payload readiness 漂移");
  return { eventId: boundedText(raw.eventId, "taskCockpit.reviewIssue.event.eventId", 200), sequence: integer(raw.sequence, "taskCockpit.reviewIssue.event.sequence", 1), eventType: enumValue(raw.eventType, ["opened", "resolved", "returned", "superseded"] as const, "taskCockpit.reviewIssue.event.eventType"), issueVersion: integer(raw.issueVersion, "taskCockpit.reviewIssue.event.issueVersion", 1), payloadHash, payload, payloadReadiness, actor: boundedText(raw.actor, "taskCockpit.reviewIssue.event.actor", 200), createdAt: timestamp(raw.createdAt, "taskCockpit.reviewIssue.event.createdAt") };
}
function parseTaskCockpitReturnLineage(value: unknown): TaskCockpitReviewReturnLineage {
  const raw = record(value, "taskCockpit.reviewIssue.returnLineage"); exact(raw, ["decisionId", "issueVersion", "runId", "stepKey", "stepRunId", "attempt", "decisionHash", "impactDecisions", "impactReadiness", "createdAt"], "taskCockpit.reviewIssue.returnLineage"); const decisionHash = boundedText(raw.decisionHash, "taskCockpit.reviewIssue.returnLineage.decisionHash", 64); if (!RAW_SHA256.test(decisionHash) || !Array.isArray(raw.impactDecisions)) throw new TypeError("taskCockpit.reviewIssue decision 漂移");
  const impactDecisions = raw.impactDecisions.map((value, index) => { const item = record(value, `taskCockpit.reviewIssue.impactDecisions[${index}]`); exact(item, ["stepKey", "action", "reason"], `taskCockpit.reviewIssue.impactDecisions[${index}]`); return { stepKey: boundedText(item.stepKey, "taskCockpit.reviewIssue.impact.stepKey", 160), action: enumValue(item.action, ["invalidate", "reuse"] as const, "taskCockpit.reviewIssue.impact.action"), reason: boundedText(item.reason, "taskCockpit.reviewIssue.impact.reason", 500) }; }); const impactReadiness = enumValue(raw.impactReadiness, ["exact", "legacy_unavailable"] as const, "taskCockpit.reviewIssue.impactReadiness"); if ((impactReadiness === "exact") !== (impactDecisions.length > 0)) throw new TypeError("taskCockpit.reviewIssue impact readiness 漂移");
  return { decisionId: boundedText(raw.decisionId, "taskCockpit.reviewIssue.returnLineage.decisionId", 200), issueVersion: integer(raw.issueVersion, "taskCockpit.reviewIssue.returnLineage.issueVersion", 1), runId: boundedText(raw.runId, "taskCockpit.reviewIssue.returnLineage.runId", 200), stepKey: boundedText(raw.stepKey, "taskCockpit.reviewIssue.returnLineage.stepKey", 200), stepRunId: boundedText(raw.stepRunId, "taskCockpit.reviewIssue.returnLineage.stepRunId", 200), attempt: integer(raw.attempt, "taskCockpit.reviewIssue.returnLineage.attempt", 1), decisionHash, impactDecisions, impactReadiness, createdAt: timestamp(raw.createdAt, "taskCockpit.reviewIssue.returnLineage.createdAt") };
}
...
function parseCreatorPrepareRef(value: unknown, label: string): CreatorPrepareExactRef { const raw = record(value, label); exact(raw, ["resourceType", "resourceId", "revision", "contentHash"], label); const contentHash = boundedText(raw.contentHash, `${label}.contentHash`, 64); if (!RAW_SHA256.test(contentHash)) throw new TypeError(`${label}.contentHash 非 SHA-256`); return { resourceType: boundedText(raw.resourceType, `${label}.resourceType`, 120), resourceId: boundedText(raw.resourceId, `${label}.resourceId`, 200), revision: integer(raw.revision, `${label}.revision`, 1), contentHash }; }
function parseCreatorPrepareItem(value: unknown, label: string): CreatorPrepareBatchItem { const raw = record(value, label); exact(raw, ["candidateRef", "evidenceBundleRef", "matchObservationRef", "matchDecisionRef", "disposition", "reasonCodes", "frequencyEligible", "capacityEligible", "budgetEligible"], label); if (!Array.isArray(raw.reasonCodes) || typeof raw.frequencyEligible !== "boolean" || typeof raw.capacityEligible !== "boolean" || typeof raw.budgetEligible !== "boolean") throw new TypeError(`${label} contract 漂移`); return { candidateRef: parseCreatorPrepareRef(raw.candidateRef, `${label}.candidateRef`), evidenceBundleRef: parseCreatorPrepareRef(raw.evidenceBundleRef, `${label}.evidenceBundleRef`), matchObservationRef: parseCreatorPrepareRef(raw.matchObservationRef, `${label}.matchObservationRef`), matchDecisionRef: parseCreatorPrepareRef(raw.matchDecisionRef, `${label}.matchDecisionRef`), disposition: enumValue(raw.disposition, ["eligible", "excluded", "needs_review", "unknown", "deduplicated"] as const, `${label}.disposition`), reasonCodes: raw.reasonCodes.map((item) => boundedText(item, `${label}.reasonCode`, 120)), frequencyEligible: raw.frequencyEligible, capacityEligible: raw.capacityEligible, budgetEligible: raw.budgetEligible }; }
function parseCreatorPreparation(value: unknown, expectedTenant: WorkshopTenant): CreatorBatchPreparation { const raw = record(value, "creatorPrepare.batch"); exact(raw, ["tenant", "batchId", "revision", "version", "lifecycle", "exactRefs", "skillRefs", "items", "itemHashes", "ledger", "externalEffectCount", "actionProposalCount", "executionLeaseCount", "priorContentHash", "contentHash", "createdBy", "createdAt"], "creatorPrepare.batch"); const tenant = parseTenant(raw.tenant); if (tenant.orgId !== expectedTenant.orgId || tenant.projectId !== expectedTenant.projectId || !Array.isArray(raw.skillRefs) || !Array.isArray(raw.items) || !Array.isArray(raw.itemHashes)) throw new TypeError("creatorPrepare batch tenant/array 漂移"); const exactRaw = record(raw.exactRefs, "creatorPrepare.exactRefs"); const exactRefs = Object.fromEntries(Object.entries(exactRaw).map(([key, ref]) => [key, parseCreatorPrepareRef(ref, `creatorPrepare.exactRefs.${key}`)])); const skillRefs = raw.skillRefs.map((ref, index) => parseCreatorPrepareRef(ref, `creatorPrepare.skillRefs[${index}]`)); const items = raw.items.map((item, index) => parseCreatorPrepareItem(item, `creatorPrepare.items[${index}]`)); assertUnique(skillRefs.map((ref) => ref.resourceId), "creatorPrepare.skillRefs"); assertUnique(items.map((item) => item.candidateRef.resourceId), "creatorPrepare.items"); const itemHashes = raw.itemHashes.map((item) => { const result = boundedText(item, "creatorPrepare.itemHash", 64); if (!RAW_SHA256.test(result)) throw new TypeError("creatorPrepare.itemHash 非 SHA-256"); return result; }); assertUnique(itemHashes, "creatorPrepare.itemHashes"); if (items.length !== 0 && items.length !== itemHashes.length) throw new TypeError("creatorPrepare items/hash 数量漂移"); const ledger = parseCreatorLedger(raw.ledger); const contentHash = boundedText(raw.contentHash, "creatorPrepare.contentHash", 64); if (!RAW_SHA256.test(contentHash) || raw.externalEffectCount !== 0 || raw.actionProposalCount !== 0 || raw.executionLeaseCount !== 0) throw new TypeError("creatorPrepare 非零外部副作用或 hash 漂移"); const priorContentHash = raw.priorContentHash === null ? null : boundedText(raw.priorContentHash, "creatorPrepare.priorContentHash", 64); if (priorContentHash !== null && !RAW_SHA256.test(priorContentHash)) throw new TypeError("creatorPrepare.priorContentHash 非 SHA-256"); return { tenant, batchId: boundedText(raw.batchId, "creatorPrepare.batchId", 200), revision: integer(raw.revision, "creatorPrepare.revision", 1), version: integer(raw.version, "creatorPrepare.version", 1), lifecycle: enumValue(raw.lifecycle, ["prepared", "frozen", "stale"] as const, "creatorPrepare.lifecycle"), exactRefs, skillRefs, items, itemHashes, ledger, externalEffectCount: 0, actionProposalCount: 0, executionLeaseCount: 0, priorContentHash, contentHash, createdBy: boundedText(raw.createdBy, "creatorPrepare.createdBy", 200), createdAt: timestamp(raw.createdAt, "creatorPrepare.createdAt") }; }
export function parseCreatorContributionView(value: unknown, expectedTenant?: WorkshopTenant): CreatorContributionViewResponse { const raw = record(value, "creatorPrepare"); exact(raw, ["schemaVersion", "tenant", "evaluatedAt", "atomicSkillRefs", "logicRef", "primaryColleague", "collaboratorColleagues", "latestBatch", "blockers", "allowedCommands", "externalEffectsAllowed"], "creatorPrepare"); const tenant = parseTenant(raw.tenant); if (raw.schemaVersion !== CREATOR_PREPARE_SCHEMA_VERSION || (expectedTenant && (tenant.orgId !== expectedTenant.orgId || tenant.projectId !== expectedTenant.projectId)) || raw.primaryColleague !== "导购顾问" || raw.externalEffectsAllowed !== false || !Array.isArray(raw.atomicSkillRefs) || !Array.isArray(raw.collaboratorColleagues) || !Array.isArray(raw.blockers) || !Array.isArray(raw.allowedCommands)) throw new TypeError("creatorPrepare contract 漂移"); const atomicSkillRefs = raw.atomicSkillRefs.map((ref, index) => parseCreatorPrepareRef(ref, `creatorPrepare.atomicSkillRefs[${index}]`)); const logicRef = raw.logicRef === null ? null : parseCreatorPrepareRef(raw.logicRef, "creatorPrepare.logicRef"); const allowedCommands = raw.allowedCommands.map((command) => enumValue(command, ["PREPARE_CREATOR_BATCH", "FREEZE_CREATOR_BATCH"] as const, "creatorPrepare.allowedCommand")); assertUnique(atomicSkillRefs.map((ref) => ref.resourceId), "creatorPrepare.atomicSkillRefs"); assertUnique(allowedCommands, "creatorPrepare.allowedCommands"); const blockers = raw.blockers.map((item) => boundedText(item, "creatorPrepare.blocker", 200)); const latestBatch = raw.latestBatch === null ? null : parseCreatorPreparation(raw.latestBatch, tenant); if ((latestBatch === null) !== blockers.includes("CREATOR_BATCH_PREPARATION_NOT_AVAILABLE") || (latestBatch?.lifecycle === "frozen" && blockers.length > 0)) throw new TypeError("creatorPrepare batch/blocker 漂移"); return { schemaVersion: CREATOR_PREPARE_SCHEMA_VERSION, tenant, evaluatedAt: timestamp(raw.evaluatedAt, "creatorPrepare.evaluatedAt"), atomicSkillRefs, logicRef, primaryColleague: "导购顾问", collaboratorColleagues: raw.collaboratorColleagues.map((item) => boundedText(item, "creatorPrepare.collaborator", 80)), latestBatch, blockers, allowedCommands, externalEffectsAllowed: false }; }

const CREATOR_ACTION_KINDS = ["outreach-message", "sample-shipment", "contract-signature", "commission-change"] as const satisfies readonly CreatorActionKind[];
const CREATOR_LANE_OUTCOMES = ["prepared", "accepted", "applied", "failed", "unknown", "disputed"] as const;
function parseCreatorStartLedger(value: unknown): CreatorStartLedger { const raw = record(value, "creatorLifecycle.ledger"); exact(raw, ["eligibleItems", "lanes", "prepared", "accepted", "applied", "failed", "unknown", "disputed", "completed"], "creatorLifecycle.ledger"); const result = { eligibleItems: integer(raw.eligibleItems, "creatorLifecycle.ledger.eligibleItems"), lanes: integer(raw.lanes, "creatorLifecycle.ledger.lanes"), prepared: integer(raw.prepared, "creatorLifecycle.ledger.prepared"), accepted: integer(raw.accepted, "creatorLifecycle.ledger.accepted"), applied: integer(raw.applied, "creatorLifecycle.ledger.applied"), failed: integer(raw.failed, "creatorLifecycle.ledger.failed"), unknown: integer(raw.unknown, "creatorLifecycle.ledger.unknown"), disputed: integer(raw.disputed, "creatorLifecycle.ledger.disputed"), completed: integer(raw.completed, "creatorLifecycle.ledger.completed") }; if (result.lanes !== result.prepared + result.accepted + result.applied + result.failed + result.unknown + result.disputed || result.completed !== result.applied + result.failed) throw new TypeError("creatorLifecycle ledger 不守恒"); return result; }
function parseCreatorLane(value: unknown, label: string): CreatorActionLaneBinding { const raw = record(value, label); exact(raw, ["candidateRef", "actionKind", "actionTypeRef", "impactPreviewRef", "adapterCapabilityRef", "accountBindingRef", "budgetReservationRef", "approvalPolicyRef", "laneId", "bindingHash", "outcome", "actionReceiptRef", "resolvedOutcome"], label); const actionKind = enumValue(raw.actionKind, CREATOR_ACTION_KINDS, `${label}.actionKind`); const actionTypeRef = parseCreatorPrepareRef(raw.actionTypeRef, `${label}.actionTypeRef`); const bindingHash = hash(raw.bindingHash, `${label}.bindingHash`); if (actionTypeRef.resourceId !== `ecommerce.creator.${actionKind}`) throw new TypeError(`${label} action type 漂移`); return { candidateRef: parseCreatorPrepareRef(raw.candidateRef, `${label}.candidateRef`), actionKind, actionTypeRef, impactPreviewRef: parseCreatorPrepareRef(raw.impactPreviewRef, `${label}.impactPreviewRef`), adapterCapabilityRef: parseCreatorPrepareRef(raw.adapterCapabilityRef, `${label}.adapterCapabilityRef`), accountBindingRef: parseCreatorPrepareRef(raw.accountBindingRef, `${label}.accountBindingRef`), budgetReservationRef: parseCreatorPrepareRef(raw.budgetReservationRef, `${label}.budgetReservationRef`), approvalPolicyRef: parseCreatorPrepareRef(raw.approvalPolicyRef, `${label}.approvalPolicyRef`), laneId: boundedText(raw.laneId, `${label}.laneId`, 240), bindingHash, outcome: enumValue(raw.outcome, CREATOR_LANE_OUTCOMES, `${label}.outcome`), actionReceiptRef: raw.actionReceiptRef === null ? null : parseCreatorPrepareRef(raw.actionReceiptRef, `${label}.actionReceiptRef`), resolvedOutcome: raw.resolvedOutcome === null ? null : enumValue(raw.resolvedOutcome, CREATOR_LANE_OUTCOMES, `${label}.resolvedOutcome`) }; }
function parseCreatorStart(value: unknown, tenant: WorkshopTenant): CreatorBatchStartDecision { const raw = record(value, "creatorLifecycle.start"); exact(raw, ["tenant", "decisionId", "revision", "version", "lifecycle", "batchRef", "batchItemHashes", "lanes", "ledger", "operationalBlockers", "externalEffectCount", "contentHash", "startedBy", "startedAt"], "creatorLifecycle.start"); const ownTenant = parseTenant(raw.tenant); if (ownTenant.orgId !== tenant.orgId || ownTenant.projectId !== tenant.projectId || raw.lifecycle !== "governance_prepared" || raw.externalEffectCount !== 0 || !Array.isArray(raw.batchItemHashes) || !Array.isArray(raw.lanes) || !Array.isArray(raw.operationalBlockers)) throw new TypeError("creatorLifecycle start contract 漂移"); const lanes = raw.lanes.map((item, index) => parseCreatorLane(item, `creatorLifecycle.lanes[${index}]`)); assertUnique(lanes.map((item) => item.laneId), "creatorLifecycle.lanes"); const ledger = parseCreatorStartLedger(raw.ledger); if (ledger.lanes !== lanes.length) throw new TypeError("creatorLifecycle lanes/ledger 漂移"); return { tenant: ownTenant, decisionId: boundedText(raw.decisionId, "creatorLifecycle.decisionId", 200), revision: integer(raw.revision, "creatorLifecycle.revision", 1), version: integer(raw.version, "creatorLifecycle.version", 1), lifecycle: "governance_prepared", batchRef: parseCreatorPrepareRef(raw.batchRef, "creatorLifecycle.batchRef"), batchItemHashes: raw.batchItemHashes.map((item) => hash(item, "creatorLifecycle.batchItemHash")), lanes, ledger, operationalBlockers: raw.operationalBlockers.map((item) => boundedText(item, "creatorLifecycle.blocker", 200)), externalEffectCount: 0, contentHash: hash(raw.contentHash, "creatorLifecycle.contentHash"), startedBy: boundedText(raw.startedBy, "creatorLifecycle.startedBy", 200), startedAt: timestamp(raw.startedAt, "creatorLifecycle.startedAt") }; }
function parseCreatorContract(value: unknown, tenant: WorkshopTenant): CreatorContractRevision { const raw = record(value, "creatorLifecycle.contract"); exact(raw, ["tenant", "collaborationId", "revision", "candidateRef", "startDecisionRef", "priorContractRef", "termsHash", "monetaryTermsRef", "lifecycle", "diffFields", "actionReceiptRef", "contentHash", "createdBy", "createdAt"], "creatorLifecycle.contract"); const own = parseTenant(raw.tenant); if (own.orgId !== tenant.orgId || own.projectId !== tenant.projectId || !Array.isArray(raw.diffFields)) throw new TypeError("creatorLifecycle contract tenant/array 漂移"); return { tenant: own, collaborationId: boundedText(raw.collaborationId, "creatorLifecycle.collaborationId", 200), revision: integer(raw.revision, "creatorLifecycle.contract.revision", 1), candidateRef: parseCreatorPrepareRef(raw.candidateRef, "creatorLifecycle.contract.candidateRef"), startDecisionRef: parseCreatorPrepareRef(raw.startDecisionRef, "creatorLifecycle.contract.startDecisionRef"), priorContractRef: raw.priorContractRef === null ? null : parseCreatorPrepareRef(raw.priorContractRef, "creatorLifecycle.contract.priorContractRef"), termsHash: hash(raw.termsHash, "creatorLifecycle.contract.termsHash"), monetaryTermsRef: parseCreatorPrepareRef(raw.monetaryTermsRef, "creatorLifecycle.contract.monetaryTermsRef"), lifecycle: enumValue(raw.lifecycle, ["draft", "reviewed", "signed_observed", "cancelled"] as const, "creatorLifecycle.contract.lifecycle"), diffFields: raw.diffFields.map((item) => boundedText(item, "creatorLifecycle.contract.diffField", 200)), actionReceiptRef: raw.actionReceiptRef === null ? null : parseCreatorPrepareRef(raw.actionReceiptRef, "creatorLifecycle.contract.actionReceiptRef"), contentHash: hash(raw.contentHash, "creatorLifecycle.contract.contentHash"), createdBy: boundedText(raw.createdBy, "creatorLifecycle.contract.createdBy", 200), createdAt: timestamp(raw.createdAt, "creatorLifecycle.contract.createdAt") }; }
function parseCreatorDelivery(value: unknown, tenant: WorkshopTenant): CreatorDeliveryObservation { const raw = record(value, "creatorLifecycle.delivery"); exact(raw, ["tenant", "observationId", "collaborationId", "contractRef", "deliveryRef", "evidenceBundleRef", "sequence", "status", "contentHash", "observedAt"], "creatorLifecycle.delivery"); const own = parseTenant(raw.tenant); if (own.orgId !== tenant.orgId || own.projectId !== tenant.projectId) throw new TypeError("creatorLifecycle delivery tenant 漂移"); return { tenant: own, observationId: boundedText(raw.observationId, "creatorLifecycle.delivery.observationId", 200), collaborationId: boundedText(raw.collaborationId, "creatorLifecycle.delivery.collaborationId", 200), contractRef: parseCreatorPrepareRef(raw.contractRef, "creatorLifecycle.delivery.contractRef"), deliveryRef: parseCreatorPrepareRef(raw.deliveryRef, "creatorLifecycle.delivery.deliveryRef"), evidenceBundleRef: parseCreatorPrepareRef(raw.evidenceBundleRef, "creatorLifecycle.delivery.evidenceBundleRef"), sequence: integer(raw.sequence, "creatorLifecycle.delivery.sequence", 1), status: enumValue(raw.status, ["expected", "in_transit", "received", "accepted", "disputed"] as const, "creatorLifecycle.delivery.status"), contentHash: hash(raw.contentHash, "creatorLifecycle.delivery.contentHash"), observedAt: timestamp(raw.observedAt, "creatorLifecycle.delivery.observedAt") }; }
function parseCreatorRelationship(value: unknown, tenant: WorkshopTenant): CreatorRelationshipRevision { const raw = record(value, "creatorLifecycle.relationship"); exact(raw, ["tenant", "relationshipId", "revision", "collaborationId", "contractRef", "deliveryObservationRefs", "maturityPolicyRef", "maturityWindowEndsAt", "status", "contentHash", "createdAt"], "creatorLifecycle.relationship"); const own = parseTenant(raw.tenant); if (own.orgId !== tenant.orgId || own.projectId !== tenant.projectId || !Array.isArray(raw.deliveryObservationRefs)) throw new TypeError("creatorLifecycle relationship tenant/refs 漂移"); return { tenant: own, relationshipId: boundedText(raw.relationshipId, "creatorLifecycle.relationship.id", 200), revision: integer(raw.revision, "creatorLifecycle.relationship.revision", 1), collaborationId: boundedText(raw.collaborationId, "creatorLifecycle.relationship.collaborationId", 200), contractRef: parseCreatorPrepareRef(raw.contractRef, "creatorLifecycle.relationship.contractRef"), deliveryObservationRefs: raw.deliveryObservationRefs.map((item, index) => parseCreatorPrepareRef(item, `creatorLifecycle.relationship.deliveryRefs[${index}]`)), maturityPolicyRef: parseCreatorPrepareRef(raw.maturityPolicyRef, "creatorLifecycle.relationship.maturityPolicyRef"), maturityWindowEndsAt: timestamp(raw.maturityWindowEndsAt, "creatorLifecycle.relationship.maturityWindowEndsAt"), status: enumValue(raw.status, ["preliminary", "mature", "at_risk"] as const, "creatorLifecycle.relationship.status"), contentHash: hash(raw.contentHash, "creatorLifecycle.relationship.contentHash"), createdAt: timestamp(raw.createdAt, "creatorLifecycle.relationship.createdAt") }; }
export function parseCreatorLifecycleView(value: unknown, expectedTenant?: WorkshopTenant): CreatorLifecycleViewResponse { const raw = record(value, "creatorLifecycle"); exact(raw, ["schemaVersion", "tenant", "evaluatedAt", "latestStart", "ledger", "contracts", "deliveries", "relationships", "blockers", "allowedCommands", "externalEffectsAllowed"], "creatorLifecycle"); const tenant = parseTenant(raw.tenant); if (raw.schemaVersion !== CREATOR_LIFECYCLE_SCHEMA_VERSION || (expectedTenant && (tenant.orgId !== expectedTenant.orgId || tenant.projectId !== expectedTenant.projectId)) || raw.externalEffectsAllowed !== false || !Array.isArray(raw.contracts) || !Array.isArray(raw.deliveries) || !Array.isArray(raw.relationships) || !Array.isArray(raw.blockers) || !Array.isArray(raw.allowedCommands)) throw new TypeError("creatorLifecycle contract 漂移"); const latestStart = raw.latestStart === null ? null : parseCreatorStart(raw.latestStart, tenant); const ledger = parseCreatorStartLedger(raw.ledger); if ((latestStart === null) !== raw.blockers.includes("CREATOR_BATCH_START_NOT_AVAILABLE") || (latestStart && latestStart.ledger.lanes !== ledger.lanes) || raw.allowedCommands.some((item) => item !== "START_CREATOR_BATCH_GOVERNANCE")) throw new TypeError("creatorLifecycle start/blocker/command 漂移"); return { schemaVersion: CREATOR_LIFECYCLE_SCHEMA_VERSION, tenant, evaluatedAt: timestamp(raw.evaluatedAt, "creatorLifecycle.evaluatedAt"), latestStart, ledger, contracts: raw.contracts.map((item) => parseCreatorContract(item, tenant)), deliveries: raw.deliveries.map((item) => parseCreatorDelivery(item, tenant)), relationships: raw.relationships.map((item) => parseCreatorRelationship(item, tenant)), blockers: raw.blockers.map((item) => boundedText(item, "creatorLifecycle.blocker", 200)), allowedCommands: raw.allowedCommands as "START_CREATOR_BATCH_GOVERNANCE"[], externalEffectsAllowed: false }; }
