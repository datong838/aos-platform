...
export function BusinessInvestigationTab({ id, labelledBy, client = ecommerceInvestigationClient }: { id: string; labelledBy: string; client?: InvestigationReadClient }) {
  ...
  const caseRequest = useRef(0); const runRequest = useRef(0);
  ...
  const clearRuns = () => { runRequest.current += 1; setRuns([]); setSelectedRunId(""); setRunPhase("idle"); };
  ...
  useEffect(() => { loadCases(); return () => { caseRequest.current += 1; runRequest.current += 1; }; }, [client]);
  useEffect(() => {
  ...
  return (
    <section id={id} aria-labelledby={labelledBy} className="analyst-panel business-investigation-tab" role="tabpanel">
      <header><div><span>Business Investigation · BI-W7-02</span><h2>生意探究</h2></div><strong className="content-campaign-status is-blocked">只读</strong></header>
      ...
      {phase === "ready" && selectedCase ? <section className="business-investigation-current" aria-label="当前分析记录">
        ...
        {runPhase === "ready" ? <label><span>Run 记录</span><select aria-label="Run 记录" value={selectedRunId} onChange={(event) => setSelectedRunId(event.currentTarget.value)}>{runs.map((item) => <option key={item.authority.runId} value={item.authority.runId}>{item.authority.runId} · {item.state.lifecycle}/{item.state.control}</option>)}</select></label> : null}
      </section> : null}
