...
export function ContentCampaignPage({ client = ecommerceWorkshopClient }: { client?: Client }) {
  ...
  const content = response && slice ? <div className="content-campaign-read-model">
    <section className="content-campaign-metrics" aria-label="内容活动数量守恒"><div><span>权威切片</span><strong>3</strong><small>固定 canonical order</small></div><div><span>读取可用</span><strong>{ready}</strong><small>其余失败关闭</small></div><div><span>可评估</span><strong>{eligible}</strong><small>非示例业务数</small></div><div><span>已挂接</span><strong>{response.page.count}</strong><small>等于三切片 items</small></div><div><span>待核对</span><strong>{unmatched}</strong><small>未匹配 + 冲突</small></div><div><span>数据截止</span><strong>{formatTime(response.dataCutoff)}</strong><small>统一 cutoff</small></div></section>
    <section className="content-campaign-readonly"><span aria-hidden="true">◇</span><div><h2>内容与活动 · 权威只读</h2><p>仅呈现 Campaign、Calendar、Master Intent 与 ContentVariant exact lineage；不批准、不排期、不发布。</p></div><strong>无写入口</strong></section>
    <div className="content-campaign-board">
      ...
      <main className="content-campaign-detail"><header><div><span>{slice.sliceId}</span><h2>{LABELS[slice.sliceId]}</h2></div><strong className={`content-campaign-status is-${slice.status}`}>{slice.status === "ready" ? "权威读取可用" : "失败关闭"}</strong></header><section className="content-campaign-ledger"><div><span>可评估</span><strong>{slice.countLedger.eligible}</strong></div><div><span>已挂接</span><strong>{slice.countLedger.attached}</strong></div><div><span>未匹配</span><strong>{slice.countLedger.unmatched}</strong></div><div><span>冲突</span><strong>{slice.countLedger.conflicted}</strong></div></section><section className="content-campaign-items" aria-label={`${LABELS[slice.sliceId]} exact facts`}>{slice.items.length ? slice.items.map((item) => <ItemCard item={item} key={`${item.resourceType}:${item.resourceId}:${item.revision}`} />) : <p>当前没有合格的已挂接事实；可信空与阻断由右侧证据区分。</p>}</section></main>
