  ...
  const content = response && slice ? <div className="content-campaign-read-model">
    <div className="content-campaign-board">
      ...
      <section className="content-campaign-detail" aria-label="内容与活动详情">
        <section className="content-campaign-visual-intent"><div><strong>AI 策划助手</strong><span>仅消费当前 canonical authority</span></div><div><textarea aria-label="活动意图" readOnly value="当前没有可编辑活动意图；请选择具有 exact authority 的计划" /><button type="button" disabled>生成方案</button></div></section>
        <section className={`content-campaign-assistant-summary is-${slice.status}`} aria-label="AI 策划结论"><div><strong>{slice.status === "ready" ? "当前权威事实可供审阅" : "尚未形成可审阅方案"}</strong><span>{slice.status === "ready" ? "只读呈现 exact facts，不代表已批准或发布。" : "canonical authority 未闭合，助手不生成活动、预算、经营指标或风险建议。"}</span></div><em>{slice.status === "ready" ? "只读" : "等待 authority"}</em></section>
        <section className="content-campaign-overview" aria-label="活动概览"><header><div><span>{slice.sliceId}</span><h2>活动概览</h2></div><strong className={`content-campaign-status is-${slice.status}`}>{slice.status === "ready" ? "权威读取可用" : "失败关闭"}</strong></header><p className="content-campaign-overview-title"><strong>{LABELS[slice.sliceId]}</strong><span>{slice.status === "ready" ? "exact facts 已挂接" : "等待 exact authority"}</span></p><section className="content-campaign-ledger"><div><span>可评估</span><strong>{slice.countLedger.eligible}</strong></div><div><span>已挂接</span><strong>{slice.countLedger.attached}</strong></div><div><span>未匹配</span><strong>{slice.countLedger.unmatched}</strong></div><div><span>冲突</span><strong>{slice.countLedger.conflicted}</strong></div></section><section className="content-campaign-items" aria-label={`${LABELS[slice.sliceId]} exact facts`}>{slice.items.length ? slice.items.map((item) => <ItemCard item={item} key={`${item.resourceType}:${item.resourceId}:${item.revision}`} />) : <p>当前没有合格的已挂接事实；可信空与阻断由右侧证据区分。</p>}</section></section>
        <footer className="content-campaign-actionbar" aria-label="活动审批操作"><div><strong>{slice.status === "ready" ? "只读审阅" : "等待 exact authority"}</strong><span>未获得 Draft、Approval 与 Release authority，所有写操作保持禁用。</span></div><div><button type="button" disabled>保存为草稿</button><button type="button" disabled>重新生成</button><button type="button" disabled>批准并发布</button></div></footer>
      </section>
