...
function CustomerFailureSurface() {
  return <div className="customer-read-model is-failed-read">
    <div className="customer-tabs customer-failure-taskrail" role="tablist" aria-label="客户关系只读视图"><header><strong>今日任务与客户分群</strong><span>reader 失败 · 可信空</span></header>{Object.entries(LABELS).map(([viewId, label], index) => <button id={`customer-tab-${viewId}`} type="button" role="tab" aria-selected={index === 0} tabIndex={index === 0 ? 0 : -1} disabled key={viewId}><strong>{label}</strong><span>未装载</span></button>)}<section className="customer-failure-segments"><strong>客户分群（RFM）</strong><div><span>高价值 unknown</span><span>潜力 unknown</span><span>活跃 unknown</span><span>沉睡 unknown</span></div><small>没有 exact segment authority，不显示人数。</small></section></div>
    <section className="customer-panel" role="tabpanel" aria-label="客户最小投影"><header><div><span>customer</span><h2>客户最小投影</h2></div><strong className="content-campaign-status is-blocked">读取失败</strong></header>
      <div className="customer-readiness customer-failure-plan"><header><div><span>PLAN MODE</span><h2>Plan Mode · 只读执行计划</h2></div><strong>无法编译</strong></header><p>正式服务未返回可验证客户投影；不以演示客户、联系方式或 0 值补齐。</p>{Object.entries(AXIS_LABELS).map(([axis, label], index) => <article className="is-unknown" key={axis}><i>{index + 1}</i><div><strong>{label}</strong><span>unknown · 无 exact authority</span></div></article>)}<div className="customer-failure-actions"><button type="button" disabled>暂停计划</button><button type="button" disabled>重新执行</button><small>没有 canonical Plan/TaskRun，操作保持禁用。</small></div></div>
      <section className="customer-items" aria-hidden="true" />
      <aside className="media-studio-blockers customer-failure-artifacts"><section><header><span>ARTIFACT</span><h3>Artifact 产出</h3></header><strong>当前没有可回链产物</strong><p>话术、名单与客户 360° 均未从 canonical reader 返回。</p></section><section><header><span>SEGMENT</span><h3>客户分层结果</h3></header><div className="customer-failure-segment-grid"><span>高价值<br /><b>unknown</b></span><span>潜力<br /><b>unknown</b></span><span>活跃<br /><b>unknown</b></span><span>沉睡<br /><b>unknown</b></span></div></section><section><header><span>BLOCKER</span><h3>依赖与下一证据</h3></header><div><strong>CUSTOMER_VIEW_READ_FAILED</strong><span>canonical customer reader</span><p>重新读取同租户、同截止面的隐私最小化投影。</p></div></section></aside>
    </section>
  </div>;
}
