...
import { EcommerceWorkshopClientError, type TaskCockpitActionReceiptResponse, type TaskCockpitApprovalReviewResponse, type TaskCockpitCoreResponse, type TaskCockpitProductionContextResponse, type TaskCockpitResponsibilityHandoffResponse, type TaskCockpitSkillContributionResponse } from "../../api/ecommerceWorkshop";
...
const unreadDetails = { listTaskCockpitRunSteps: vi.fn(), listTaskCockpitRunCheckpoints: vi.fn(), getTaskCockpitRunProductionContext: vi.fn(), getTaskCockpitRunResponsibilityHandoffs: vi.fn(), getTaskCockpitRunApprovalReview: vi.fn(), getTaskCockpitRunActionReceipts: vi.fn(), getTaskCockpitRunSkillContributions: vi.fn() };
...
describe("TaskCockpitPage", () => {
  ...
  it("显式展开 Run 后诚实显示 Step/Checkpoint 空权威集合", async () => {
    ...
    expect(host.textContent).toContain("职责与交接 · standard"); expect(host.textContent).toContain("观测时已解析"); expect(host.textContent).toContain("Resolution Receipt 无 expiry"); expect(host.textContent).toContain("agent-collector → agent-review");
  ...
  it("专业 Skill 贡献接口失败不拖垮原有六类运行明细", async () => {
    const client = { getTaskCockpitCore: vi.fn().mockResolvedValue(core()), listTaskCockpitRunSteps: vi.fn().mockResolvedValue(detailBase), listTaskCockpitRunCheckpoints: vi.fn().mockResolvedValue(detailBase), getTaskCockpitRunProductionContext: vi.fn().mockResolvedValue(productionContext), getTaskCockpitRunResponsibilityHandoffs: vi.fn().mockResolvedValue(responsibilityHandoffs), getTaskCockpitRunApprovalReview: vi.fn().mockResolvedValue(approvalReview), getTaskCockpitRunActionReceipts: vi.fn().mockResolvedValue(actionReceipts), getTaskCockpitRunSkillContributions: vi.fn().mockRejectedValue(new Error("offline")) };
    await act(async () => root.render(<TaskCockpitPage client={client} />));
    const button = [...host.querySelectorAll("button")].find((item) => item.textContent === "查看运行明细")!;
    await act(async () => button.click());
