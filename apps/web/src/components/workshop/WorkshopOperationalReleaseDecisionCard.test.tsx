...
describe("WorkshopOperationalReleaseDecisionCard", () => {
  ...
  it("renders eight honest blocked capabilities, NO_GO and no state-changing controls", async () => {
    ...
    expect(host.textContent).toContain("NO_GO · 失败关闭"); expect(host.textContent).toContain("0 / 8"); expect(host.textContent).toContain("8 blocked");
    expect(host.textContent).toContain("价格治理"); expect(host.textContent).toContain("客户运营"); expect(host.textContent).toContain("W8-11");
