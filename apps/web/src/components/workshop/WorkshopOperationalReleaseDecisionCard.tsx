...
export function WorkshopOperationalReleaseDecisionCard({ evidence = {} }: { evidence?: OperationalReleaseEvidence }) {
  ...
  return <section className={`workshop-operational-release-decision is-${result.decision}`} aria-label="运营就绪与发布决定" data-testid="workshop-operational-release-decision" style={{ display: "grid", gap: 12, padding: 16, border: "1px solid var(--aos-border)", borderLeft: "4px solid var(--aos-red-600, #dc2626)", background: "color-mix(in srgb, var(--aos-surface) 96%, #fff7ed)" }}>
    <header style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16, flexWrap: "wrap" }}><div style={{ display: "grid", gap: 4 }}><span style={{ color: "var(--aos-text-tertiary)", fontSize: 11, letterSpacing: ".08em" }}>发布决定 · 只读</span><h2 style={{ margin: 0 }}>逐项能力运营就绪与发布决定</h2></div><strong>{result.decision === "go" ? "满足发布条件（仅合同判定）" : "暂不发布"}</strong></header>
    ...
    <div aria-label="发布决定权威状态" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 8 }}>
      ...
      <div style={cell}><span>累计安全检查</span><strong>{result.cumulativeState}</strong></div>
    ...
    <footer>批准候选、开启功能、开始灰度、停止、回滚、故障前滚与发布：全部禁用 · 无真实租户变更 · 无外部副作用 · 无发布</footer>
  </section>;
}
