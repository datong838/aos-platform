...
export function PublishPage() {
  ...
  useEffect(() => {
    apiGet<{ items?: ModuleListItem[] }>("/v1/modules")
      .then((r) => {
        ...
        setModules(items);
        if (items.length > 0) {
          setModuleId((prev) => prev || items[0].id);
        }
      })
  ...
  useEffect(() => {
    ...
    apiGet<{ items?: DeploymentItem[] }>(
      `/v1/modules/${encodeURIComponent(moduleId)}/deployments`,
    )
  ...
  async function onPublish() {
    ...
    try {
      ...
      const once = await fetch(`${getApiBase()}/v1/modules/${encodeURIComponent(mid)}/publish`, {
        method: "POST",
        headers,
        body: "{}",
      });
      if (!once.ok) {
        const errBody = await once.json().catch(() => ({}));
        throw new Error(
          (errBody as { message?: string }).message || `HTTP ${once.status}`,
        );
      }
      const pub1 = (await once.json()) as {
        publish?: { status?: string; channel?: string };
        idempotentReplay?: boolean;
      };
      const twice = await fetch(`${getApiBase()}/v1/modules/${encodeURIComponent(mid)}/publish`, {
        method: "POST",
        headers,
        body: "{}",
      });
      const pub2 = (await twice.json()) as { idempotentReplay?: boolean };

      try {
        await apiPost(`/v1/modules/${encodeURIComponent(mid)}/deploy`, {
          environment: env,
          version: "1.0.0",
          configSnapshot: { channel: env, via: "workshop-publish" },
        });
        const dep = await apiGet<{ items?: DeploymentItem[] }>(
          `/v1/modules/${encodeURIComponent(mid)}/deployments`,
        );
        setDeployments(dep.items || []);
      } catch {
        /* deploy 可选；publish 已成功 */
      }
      ...
      setPubState("err");
      setLastPub(null);
      setMsg(formatPublishResult({ ok: false, error: String((e as Error).message || e) }));
    } finally {
      setBusy(false);
    }
  }
          ...
          {lastPub && (
            ...
            >
              ...
              <div style={{ fontSize: 11, color: "#6B7280", lineHeight: 1.8 }}>
                ...
                <div>
                  环境: <span style={{ color: "#374151" }}>{env}</span>
                </div>
                <div>
                  API channel:{" "}
