...
export function AipAnalystPage() {
  ...
  async function run() { const query = buildGovernedQuery({ kind, objectType, prompt, cutoffAt: new Date().toISOString(), taskRef, skillRef, metricRef }); if (!query || busy) return; setBusy(true); setError(null); setResult(null); try { setResult(await queryAnalyst(query)); } catch (cause) { setError(cause instanceof Error ? cause.message : String(cause)); } finally { setBusy(false); } }
  return <PageChrome title="AIP 分析师" lede="受治理语义 / 知识 / 指标查询 · 单一 Result revision · 不生成演示数据"><div style={{ display: "flex", gap: 8, marginBottom: 12 }}><button onClick={() => setLeftOpen((v) => !v)}>{leftOpen ? "收起查询" : "展开查询"}</button><button onClick={() => setRightOpen((v) => !v)}>{rightOpen ? "收起证据" : "展开证据"}</button><button onClick={() => setFocus((v) => !v)}>{focus ? "退出专注" : "Focus Mode"}</button></div><div style={{ display: "grid", gridTemplateColumns: `${focus || !leftOpen ? "0" : "280px"} minmax(0,1fr) ${focus || !rightOpen ? "0" : "320px"}`, gap: 16, minHeight: 620 }}><aside style={{ ...panel, overflow: "hidden", display: focus || !leftOpen ? "none" : "block" }}><h3>查询构造器</h3><label style={label}>查询类型<select value={kind} onChange={(e) => setKind(e.target.value as QueryKind)} style={input}><option value="semantic">语义对象</option><option value="knowledge">行业知识</option><option value="metric">权威指标</option></select></label>{kind === "semantic" && <label style={label}>Object Type<select value={objectType} onChange={(e) => setObjectType(e.target.value)} style={input}><option value="Order">订单</option><option value="Product">商品</option><option value="CustomerLite">客户</option><option value="Payment">支付</option><option value="Shipment">发货</option></select></label>}{kind === "knowledge" && <label style={label}>知识问题<textarea value={prompt} onChange={(e) => setPrompt(e.target.value)} style={{ ...input, minHeight: 100 }} /></label>}<button onClick={() => void run()} disabled={Boolean(disabled) || busy} title={disabled || undefined} style={{ ...primary, opacity: disabled || busy ? .5 : 1 }}>{busy ? "查询中…" : "运行真实查询"}</button>{disabled && <p style={warning}>{disabled}</p>}<p style={muted}>不接受任意 SQL；租户由登录 Principal 决定。</p></aside><main style={panel}>{error && <div style={warning}>请求失败：{error}。未生成本地结果。</div>}<div style={{ display: "flex", justifyContent: "space-between", marginBottom: 12 }}><div>{(["table", "chart", "map", "raw"] as View[]).map((item) => <button key={item} onClick={() => setView(item)} style={view === item ? primary : undefined}>{({ table: "表格", chart: "图表", map: "地图", raw: "Raw" } as const)[item]}</button>)}</div>{result && <strong>{result.status} · {result.rows.length} 行 · r{result.revision}</strong>}</div>{result ? <ResultBody result={result} view={view} /> : <Empty title="尚未运行查询" text="选择治理查询类型后读取真实 authority；服务不可用时保持失败关闭。" />}</main><aside style={{ ...panel, overflow: "auto", display: focus || !rightOpen ? "none" : "block" }}><h3>证据详情</h3><Evidence result={result} /></aside></div></PageChrome>;
}
