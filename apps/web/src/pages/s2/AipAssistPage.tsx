...
export function recentTaskBusinessTitle(title: string): string {
  const withoutInternalWave = String(title || "")
    .replace(/^\s*(?:(?:R\d[\w.-]*|D\d[\w.-]*|W-?[A-Z0-9][\w.-]*|P\d[\w.-]*|AIP-[A-Z0-9.-]+)\s*[·:：—-]?\s*)+/i, "")
    .replace(/\bv\d+\b/gi, "")
    .replace(/\bProvider\b/gi, "模型服务")
    .replace(/\s+/g, " ")
    .trim();
  return businessDisplayName(withoutInternalWave, "历史真实任务");
}
function recentTasksFromResponse(value: unknown): RecentTask[] {
  if (!value || typeof value !== "object" || !Array.isArray((value as { items?: unknown }).items)) return [];
  return (value as { items: unknown[] }).items.flatMap((raw) => {
    if (!raw || typeof raw !== "object") return [];
    const item = raw as Record<string, unknown>;
    const id = String(item.id || "").trim(), title = String(item.title || "").trim(), status = String(item.status || "").trim();
    const version = Number(item.version), updatedAt = String(item.updatedAt || "");
    if (!id || !title || !["approved", "executing", "paused"].includes(status) || !Number.isInteger(version) || version < 1 || Number.isNaN(Date.parse(updatedAt))) return [];
    return [{ id, title, status, version, updatedAt }];
  }).sort((left, right) => Date.parse(right.updatedAt) - Date.parse(left.updatedAt)).slice(0, 5);
}
const defaultClient: AssistPageClient = {
  createThread: createAssistThread,
  streamTurn: streamAssistTurn,
  cancelTaskRun: cancelAssistTaskRun,
  newKey: newIdempotencyKey,
  listRecentTasks: async () => recentTasksFromResponse(await aipClient.request<unknown>("listTasks")),
};
...
export function AipAssistPage({ client = defaultClient }: { client?: AssistPageClient } = {}) {
  ...
  const [recentTasks, setRecentTasks] = useState<RecentTask[]>([]), [recentTasksLoading, setRecentTasksLoading] = useState(false), [recentTasksError, setRecentTasksError] = useState(""), [selectedTaskId, setSelectedTaskId] = useState("");
  ...
  const cancelDisabled = !subject ? "缺少精确运行上下文" : !taskVersion || !runVersion ? "任务或任务运行版本不能通过并发安全校验" : null;
  useEffect(() => {
    if (subject || !client.listRecentTasks) return;
    let active = true;
    setRecentTasksLoading(true);
    void client.listRecentTasks().then((items) => { if (active) { setRecentTasks(items); setRecentTasksError(""); } }).catch((cause) => { if (active) setRecentTasksError(cause instanceof Error ? cause.message : String(cause)); }).finally(() => { if (active) setRecentTasksLoading(false); });
    return () => { active = false; };
  }, [client, subject]);
  async function ensureThread(threadKey: string): Promise<AssistThread> { if (thread) return thread; if (!subject) throw new Error("缺少精确的任务、任务运行、智能体运行与数据截止时间"); const created = await client.createThread({ ...subject, title: "任务协作助手工作线程" }, threadKey); setThread(created); return created; }
  ...
  const taskRunLineagePath = subject && runVersion
    ? buildExactContextHref("/aip/lineage", {
      params: new URLSearchParams({ rootType: "task_run", rootId: subject.taskRunRef.resourceId }),
      refs: { taskRun: subject.taskRunRef },
      cutoffAt: subject.cutoffAt,
      returnTo: `${window.location.pathname}${window.location.search}`,
    })
    : null;
  const selectedTask = recentTasks.find((item) => item.id === selectedTaskId) || null;
  return <PageChrome title="任务协作助手" lede="围绕真实任务持续协作，并保留可审计的服务端事件；缺少运行上下文时明确关闭输入。">
    <div data-testid="assist-ops-stats" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(110px,1fr))", gap: 10, marginBottom: 12 }}>
      {[
        ...
        { label: "取消任务", value: cancelResult ? "已取消" : cancelDisabled ? "待补齐上下文" : "可取消" },
        { label: "输入", value: !subject ? "待补齐运行引用" : busy ? "运行中" : "可发送" },
      ].map((s) => (
    ...
    <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}><button type="button" aria-expanded={contextOpen} onClick={() => setContextOpen((v) => !v)} onKeyDown={(event) => activateToggle(event, () => setContextOpen((v) => !v))}>{contextOpen ? "收起上下文" : "展开上下文"}</button><button type="button" aria-pressed={focus} onClick={() => setFocus((v) => !v)} onKeyDown={(event) => activateToggle(event, () => setFocus((v) => !v))}>{focus ? "退出专注" : "专注模式"}</button><button type="button" onClick={() => void cancel()} disabled={Boolean(cancelDisabled) || cancelling || Boolean(cancelResult)} title={cancelDisabled || undefined}>{cancelling ? "取消中…" : cancelResult ? `已取消运行 ${cancelResult.run.id}` : "取消任务运行"}</button>{taskRunLineagePath ? <Link to={taskRunLineagePath} data-testid="assist-jump-lineage" style={ctaLink}>当前任务运行的谱系与可观测信息 →</Link> : <span data-testid="assist-lineage-blocked" title="缺少精确任务运行引用，不能猜测谱系标识" style={muted}>缺少任务运行引用，暂不能查看谱系</span>}</div><div style={{ display: "grid", gridTemplateColumns: `${focus || !contextOpen ? "0" : "320px"} minmax(0,1fr)`, gap: 16, minHeight: 620 }}><aside style={{ ...panel, display: focus || !contextOpen ? "none" : "block", overflow: "hidden" }}><h3>运行上下文</h3>{subject ? <div style={{ display: "grid", gap: 12 }}><RefFact label="任务" value={subject.taskRef} /><RefFact label="任务运行" value={subject.taskRunRef} /><RefFact label="智能体运行" value={subject.agentRunRef} /><div><strong>数据截止时间</strong><p style={muted}>{new Date(subject.cutoffAt).toLocaleString()}</p></div>{thread && <details><summary>对话线程技术标识（审计用）</summary><p style={muted}>{thread.threadId} · 版本 {thread.version}</p></details>}{cancelResult && <div role="status" style={system}>任务运行已取消 · 版本 {cancelResult.run.version}</div>}</div> : <div style={blocked} data-testid="assist-no-subject"><strong>请先选择真实任务并补齐运行引用</strong><p>请从真实任务或智能体运行工作流进入。页面不会创建默认测试智能体，也不会猜测缺失引用。</p><h4>选择最近真实任务</h4>{recentTasksLoading ? <p>正在读取真实任务…</p> : recentTasksError ? <p>真实任务读取失败：{recentTasksError}</p> : recentTasks.length ? <><select aria-label="选择最近真实任务" value={selectedTaskId} onChange={(event) => setSelectedTaskId(event.target.value)}><option value="">请选择</option>{recentTasks.map((item) => <option key={item.id} value={item.id}>{recentTaskBusinessTitle(item.title)}</option>)}</select>{selectedTask ? <div className="notice" style={{ marginTop: 8 }}><strong>{recentTaskBusinessTitle(selectedTask.title)}</strong><p>状态 {selectedTask.status} · 修订 {selectedTask.version}</p><Link to={buildExactContextHref("/aip/logic", { refs: { task: { resourceType: "Task", resourceId: selectedTask.id, revision: String(selectedTask.version), authority: "aip-task-store" } }, returnTo: "/aip/assist" })}>进入业务逻辑编排补齐运行上下文 →</Link></div> : null}</> : <p>当前没有可继续的真实任务。</p>}<p>选择任务只确定业务任务；还需从真实运行流程补齐任务运行和智能体运行引用。补齐后即可提问。</p><div style={{ display: "flex", flexWrap: "wrap", gap: 8, margin: "10px 0" }}><Link to="/aip/agents" data-testid="assist-cta-agents" style={ctaLink}>智能体列表</Link><Link to="/aip/logic" data-testid="assist-cta-logic" style={ctaLink}>业务逻辑编排</Link><Link to="/aip/agent-registry" data-testid="assist-cta-registry" style={ctaLink}>智能体目录</Link><Link to="/aip/observability" data-testid="assist-cta-observability" style={ctaLink}>可观测性</Link></div><details><summary>所需技术引用（审计用）</summary><code>taskId + taskRevision + taskAuthority<br />taskRunId + taskRunRevision + taskRunAuthority<br />agentRunId + agentRunRevision + agentRunAuthority + cutoffAt</code></details></div>}</aside><main style={{ ...panel, display: "grid", gridTemplateRows: "1fr auto", gap: 16 }}><section style={{ overflow: "auto", display: "grid", alignContent: "start", gap: 10 }}>{!events.length && !error && <div style={empty}><h3>{subject ? "准备创建权威对话线程" : "请先补齐任务运行上下文"}</h3><p>{subject ? "发送问题后只展示服务端已持久化事件。" : "当前缺少精确运行引用。请从智能体列表或业务逻辑编排进入真实工作流，补齐后即可发送问题。"}</p>{!subject && <p style={{ marginTop: 8 }}><Link to="/aip/agents" style={ctaLink}>打开智能体列表</Link></p>}</div>}{error && <div role="alert" style={blocked}><strong>请求失败</strong><br />{error}<br />未生成本地回答。</div>}{events.map((item) => <EventCard key={`${item.turnId}:${item.sequence}`} event={item} />)}</section><form onSubmit={(event) => void submit(event)} style={{ display: "flex", gap: 10 }}><input aria-label="向当前智能体运行提问" value={message} onChange={(event) => setMessage(event.target.value)} disabled={!subject || busy} title={!subject ? "缺少精确运行上下文；请从业务逻辑编排补齐任务运行与智能体运行引用" : busy ? "正在处理当前问题" : ""} placeholder={subject ? "向当前智能体提问…" : "请先从真实任务进入"} style={input} /><button type="submit" disabled={!subject || busy || message.trim().length < 2} title={!subject ? "缺少精确运行上下文；请从业务逻辑编排补齐任务运行与智能体运行引用" : undefined} style={{ ...primary, opacity: !subject || busy || message.trim().length < 2 ? .5 : 1 }}>{busy ? "运行中…" : "发送"}</button></form></main></div></PageChrome>;
}
