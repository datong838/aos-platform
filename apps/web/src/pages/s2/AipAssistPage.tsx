...
export function AipAssistPage({ client = defaultClient }: { client?: AssistPageClient } = {}) {
  ...
  return <PageChrome title="AIP 助手" lede="权威会话线程与持久化事件流；无本地答案、无隐式供应商降级">
    <div data-testid="assist-ops-stats" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(110px,1fr))", gap: 10, marginBottom: 12 }}>
      {[
        ...
        { label: "Thread", value: thread ? "已开" : "未开" },
        { label: "事件条", value: String(events.length) },
        { label: "侧栏", value: focus || !contextOpen ? "收起" : "展开" },
        { label: "取消门", value: cancelResult ? "已取消" : cancelDisabled ? "不可用" : "可取消" },
    ...
    <div style={{ display: "flex", gap: 8, marginBottom: 12 }}><button type="button" aria-expanded={contextOpen} onClick={() => setContextOpen((v) => !v)} onKeyDown={(event) => activateToggle(event, () => setContextOpen((v) => !v))}>{contextOpen ? "收起上下文" : "展开上下文"}</button><button type="button" aria-pressed={focus} onClick={() => setFocus((v) => !v)} onKeyDown={(event) => activateToggle(event, () => setFocus((v) => !v))}>{focus ? "退出专注" : "专注模式"}</button><button type="button" onClick={() => void cancel()} disabled={Boolean(cancelDisabled) || cancelling || Boolean(cancelResult)} title={cancelDisabled || undefined}>{cancelling ? "取消中…" : cancelResult ? `已取消 ${cancelResult.run.id}` : "取消 TaskRun"}</button></div><div style={{ display: "grid", gridTemplateColumns: `${focus || !contextOpen ? "0" : "320px"} minmax(0,1fr)`, gap: 16, minHeight: 620 }}><aside style={{ ...panel, display: focus || !contextOpen ? "none" : "block", overflow: "hidden" }}><h3>运行上下文</h3>{subject ? <div style={{ display: "grid", gap: 12 }}><RefFact label="Task" value={subject.taskRef} /><RefFact label="TaskRun" value={subject.taskRunRef} /><RefFact label="AgentRun" value={subject.agentRunRef} /><div><strong>cutoff</strong><p style={muted}>{new Date(subject.cutoffAt).toLocaleString()}</p></div>{thread && <><div><strong>Thread</strong><p style={muted}>{thread.threadId}</p></div><div><strong>版本</strong><p style={muted}>{thread.version}</p></div></>}{cancelResult && <div role="status" style={system}>TaskRun {cancelResult.run.status} · r{cancelResult.run.version}</div>}</div> : <div style={blocked} data-testid="assist-no-subject"><strong>尚无可执行上下文</strong><p>请从 Task/AgentRun 工作流进入，或由上游在 URL 中传递 exact refs。页面不会创建默认测试 Agent。</p><div style={{ display: "flex", flexWrap: "wrap", gap: 8, margin: "10px 0" }}><Link to="/aip/agents" data-testid="assist-cta-agents" style={ctaLink}>智能体列表</Link><Link to="/aip/logic" data-testid="assist-cta-logic" style={ctaLink}>逻辑画布</Link><Link to="/aip/agent-registry" data-testid="assist-cta-registry" style={ctaLink}>智能体目录</Link><Link to="/aip/observability" data-testid="assist-cta-observability" style={ctaLink}>可观测性</Link></div><code>taskId + taskRevision + taskAuthority<br />taskRunId + taskRunRevision + taskRunAuthority<br />agentRunId + agentRunRevision + agentRunAuthority + cutoffAt</code></div>}</aside><main style={{ ...panel, display: "grid", gridTemplateRows: "1fr auto", gap: 16 }}><section style={{ overflow: "auto", display: "grid", alignContent: "start", gap: 10 }}>{!events.length && !error && <div style={empty}><h3>{subject ? "准备创建权威 Thread" : "等待上游上下文"}</h3><p>{subject ? "发送问题后只展示服务端已持久化事件。" : "缺少 exact refs 时输入和发送保持禁用。可先从智能体列表或逻辑画布进入真实工作流。"}</p>{!subject && <p style={{ marginTop: 8 }}><Link to="/aip/agents" style={ctaLink}>打开智能体列表</Link></p>}</div>}{error && <div role="alert" style={blocked}><strong>请求失败</strong><br />{error}<br />未生成本地回答。</div>}{events.map((item) => <EventCard key={`${item.turnId}:${item.sequence}`} event={item} />)}</section><form onSubmit={(event) => void submit(event)} style={{ display: "flex", gap: 10 }}><input value={message} onChange={(event) => setMessage(event.target.value)} disabled={!subject || busy} placeholder={subject ? "向当前 AgentRun 提问…" : "请先从真实任务进入"} style={input} /><button type="submit" disabled={!subject || busy || message.trim().length < 2} title={!subject ? "缺少 exact 运行上下文" : undefined} style={{ ...primary, opacity: !subject || busy || message.trim().length < 2 ? .5 : 1 }}>{busy ? "运行中…" : "发送"}</button></form></main></div></PageChrome>;
}
