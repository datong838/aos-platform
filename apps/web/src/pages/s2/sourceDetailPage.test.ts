...
// ── D4 Phase C: Niushop 专属 Schema 已废弃 ────────────────────────

describe("D4 Phase C demoSchemaTree · niushop-mysql 已废弃", () => {
  it("niushop-mysql 不再触发专属 schema（走默认分支）", () => {
    const tree = demoSchemaTree("niushop-mysql");
    const tables = flattenTables(tree);
    // D4 Phase C: niushop-mysql 已废弃，不再返回 8 张 ns_xxx 表
    expect(tables.some((t) => t.table === "ns_order")).toBe(false);
  });

  it("jdbc-mysql-ssh 走默认分支（不绑 niushop）", () => {
