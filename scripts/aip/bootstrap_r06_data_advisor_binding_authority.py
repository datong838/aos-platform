#!/usr/bin/env python3
"""Compose the five R06 data-advisor SkillBindings without running an Agent.

The command is deliberately fail-closed.  It binds only exact published Skill
revisions, preserves the existing active data-advisor instance, and never calls
the Provider.  A binding whose capability health snapshot is stale remains in
``provisioning`` with explicit reasons instead of being presented as runnable.
"""
...
ACTOR = "aip-r06-data-advisor-binding-bootstrap"
...
def binding_id(logic_id: str) -> str:
    return f"ecommerce.data_advisor.skill.{logic_id}.r2"
...
def _dependencies(skill: Any, budget_policy_ref: VersionedAssetRef) -> OperationalBindingDependencies:
    if (
    ...
    *,
    ...
    try:
        ...
        binding, _ = registry.create_binding(
            ...
            idempotency_key=f"R06-{logic_id}-skill-binding-create-r2",
            actor=ACTOR,
    ...
    binding, readiness, _ = registry.evaluate_binding(
        ...
        idempotency_key=(
            f"R06-{logic_id}-skill-binding-evaluate-r2-v{binding.version}"
        ),
        actor=ACTOR,
    ...
    if readiness.readiness is CapabilityReadiness.AVAILABLE:
        if binding.status == "provisioning":
            binding, _ = registry.update_binding(
                ...
                idempotency_key=(
                    f"R06-{logic_id}-skill-binding-activate-r2-v{binding.version}"
                ),
                actor=ACTOR,
                occurred_at=evaluated_at,
            )
...
def apply(*, now: datetime | None = None) -> dict[str, Any]:
    ...
    if instance.status.value != "active":
        raise BindingCompositionBlocked("DATA_ADVISOR_INSTANCE_NOT_ACTIVE")
    ...
    runtime_tail = sorted(
        {
            reason
            for item in results
            for reason in item.readiness_reasons
        }
    )
    active_count = sum(item.status == "active" for item in results)
    return {
        "status": (
            "R06_DATA_ADVISOR_BINDING_GREEN"
            if active_count == len(results)
            else "R06_DATA_ADVISOR_BINDING_GREEN_WITH_RUNTIME_TAIL"
        ),
        "scope": {"orgId": SCOPE.org_id, "projectId": SCOPE.project_id},
