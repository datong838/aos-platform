#!/usr/bin/env python3
"""Plan or persist the missing R07 private-domain Logic authority chain.

The default invocation is read-only. ``--apply`` writes only P01/P03/P04/P05
...
creates a Binding, creates an AgentRun or executes customer contact.  P02 is
...
ACTOR = "aip-r07-private-domain-bootstrap"
...
def build_plan() -> dict[str, object]:
    return {
        ...
        "preservedAuthority": [P02_GRAPH_ID],
        "steps": [
            f"require exact schema head {REQUIRED_ALEMBIC_HEAD}",
            "read active exact text model alias without resolving Secret payload",
            "snapshot immutable P02 Graph and publications",
            "persist four missing draft-only Graph revisions",
            "run nine isolated governance cases per Logic",
            "persist successful dry-run and Eval evidence",
            "publish four immutable Logic revisions",
    ...
    def invoke(prompt, context):
        ...
        if "__simulate_adapter_error__" in prompt:
            raise LogicAdapterError(
                "LLM_ADAPTER_FAILED", "isolated private-domain adapter failure"
            )
        return LLMAdapterResult(
            output="isolated private-domain draft; not Provider quality evidence",
...
def apply() -> dict[str, Any]:
    ...
    if any(value for counts in before_canary.values() for value in counts.values()):
        raise RuntimeError("negative tenant canary already contains R07 authority")
    p02_before = _p02_snapshot()
    ...
    p02_after = _p02_snapshot()
    if p02_after != p02_before:
        raise RuntimeError("immutable P02 authority changed during R07 bootstrap")
    after_canary = _negative_canary_counts()
    if after_canary != before_canary:
        raise RuntimeError("R07 authority leaked into the negative tenant canary")
    return {
        "status": "R07_PRIVATE_DOMAIN_LOGIC_AUTHORITY_GREEN",
        "scope": {"orgId": SCOPE.org_id, "projectId": SCOPE.project_id},
        "model": {
