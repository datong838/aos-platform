#!/usr/bin/env python3
"""Compose four missing R07 private-domain SkillBindings without AgentRun.

The command is fail-closed. It binds only exact published Skill revisions,
preserves the existing P02 binding and active private-domain instance, and
never calls a Provider or contacts a customer. Expired capability readiness
...
def binding_id(logic_id: str) -> str:
    return f"ecommerce.private_domain_manager.skill.{logic_id}.r2"
...
def build_plan() -> dict[str, Any]:
    return {
        ...
        "preservedAuthority": [P02_BINDING_ID],
        "bindings": [
            {
                "logicId": logic_id,
...
def _snapshot_p02(registry: AipSkillRegistry) -> dict[str, Any]:
    item = registry.get_binding(SCOPE, P02_BINDING_ID)
    ...
    *,
    ...
    try:
        ...
        binding, _ = registry.create_binding(
            ...
            idempotency_key=f"R07-{logic_id}-skill-binding-create-r2",
            actor=ACTOR,
    ...
    binding, readiness, _ = registry.evaluate_binding(
        ...
        idempotency_key=(
            f"R07-{logic_id}-skill-binding-evaluate-r2-v{binding.version}"
        ),
        actor=ACTOR,
    ...
    if readiness.readiness is CapabilityReadiness.AVAILABLE:
        if binding.status == "provisioning":
            binding, _ = registry.update_binding(
                ...
                idempotency_key=(
                    f"R07-{logic_id}-skill-binding-activate-r2-v{binding.version}"
                ),
                actor=ACTOR,
                occurred_at=evaluated_at,
            )
...
def apply(*, now: datetime | None = None) -> dict[str, Any]:
    ...
    if instance.status.value != "active":
        raise BindingCompositionBlocked("PRIVATE_DOMAIN_INSTANCE_NOT_ACTIVE")
    policy = AipModelRuntimeStore().get_policy(SCOPE, POLICY_ID, POLICY_REVISION)
    registry = AipSkillRegistry()
    p02_before = _snapshot_p02(registry)
    results = [
        _ensure_binding(
            registry,
            logic_id=logic_id,
    ...
    p02_after = _snapshot_p02(registry)
    if p02_after != p02_before:
        raise BindingCompositionBlocked("IMMUTABLE_P02_BINDING_CHANGED")
    ...
    return {
        "status": (
            "R07_PRIVATE_DOMAIN_BINDING_GREEN"
            if active_count == len(results)
            else "R07_PRIVATE_DOMAIN_BINDING_GREEN_WITH_RUNTIME_TAIL"
        ),
        "scope": {"orgId": SCOPE.org_id, "projectId": SCOPE.project_id},
