#!/usr/bin/env python3
"""Plan or persist the missing R08 shopping-advisor Logic authority chain.

The default invocation is read-only. ``--apply`` writes only G01/G02/G03/G05/G06
...
creates a Binding, creates an AgentRun or executes recommendation delivery.  G04 is
...
ACTOR = "aip-r08-shopping-advisor-bootstrap"
...
def _isolated_registry(model_alias: str) -> RuntimeAdapterRegistry:
    ...
    def invoke(prompt, context):
        ...
        if "__simulate_adapter_error__" in prompt:
            raise LogicAdapterError(
                "LLM_ADAPTER_FAILED", "isolated shopping-advisor adapter failure"
            )
        return LLMAdapterResult(
            output="isolated shopping-advisor draft; not Provider quality evidence",
    ...
    registry.register_llm(
        model_alias,
        invoke,
        adapter_name="r08-shopping-advisor-isolated-contract-eval",
        read_only=True,
        dry_run_safe=True,
    )
    return registry
...
def _get_or_create_graph(logic_id: str, model_alias: str):
    ...
    request = build_shopping_advisor_graph_request(
...
def apply() -> dict[str, Any]:
    ...
    if any(value for counts in before_canary.values() for value in counts.values()):
        raise RuntimeError("negative tenant canary already contains R08 authority")
    g04_before = _g04_snapshot()
    ...
    g04_after = _g04_snapshot()
    if g04_after != g04_before:
        raise RuntimeError("immutable G04 authority changed during R08 bootstrap")
