#!/usr/bin/env python3
"""Compose five missing R08 shopping-advisor SkillBindings without AgentRun.

The command is fail-closed. It binds only exact published Skill revisions,
preserves the existing G04 binding and active shopping-advisor instance, and
never calls a Provider or delivers a recommendation. Expired capability readiness
...
ACTOR = "aip-r08-shopping-advisor-binding-bootstrap"
REQUIRED_ALEMBIC_HEAD = "aip13_001"
INSTANCE_ID = "ecommerce.shopping_advisor.default"
POLICY_ID = "policy-qyh-text-dev"
POLICY_REVISION = 2
G04_BINDING_ID = "ecommerce.shopping_advisor.skill.G04.r2"

BINDING_SPECS = {
    "G01": ["ecommerce.shared.material.collect.r1"],
    "G02": [
        "ecommerce.shared.material.collect.r1",
        "ecommerce.data_advisor.strategy.plan.r2",
    ],
    "G03": ["ecommerce.shared.copy.generate.r1"],
    "G05": ["ecommerce.shared.copy.generate.r1"],
    "G06": ["ecommerce.data_advisor.strategy.plan.r2"],
}
...
def binding_id(logic_id: str) -> str:
    return f"ecommerce.shopping_advisor.skill.{logic_id}.r2"
...
def build_plan() -> dict[str, Any]:
    return {
        ...
        "preservedAuthority": [G04_BINDING_ID],
        "bindings": [
            {
                "logicId": logic_id,
...
def _snapshot_g04(registry: AipSkillRegistry) -> dict[str, Any]:
    item = registry.get_binding(SCOPE, G04_BINDING_ID)
    ...
    *,
    ...
    try:
        ...
        binding, _ = registry.create_binding(
            ...
            idempotency_key=f"R08-{logic_id}-skill-binding-create-r2",
            actor=ACTOR,
    ...
    binding, readiness, _ = registry.evaluate_binding(
        ...
        idempotency_key=(
            f"R08-{logic_id}-skill-binding-evaluate-r2-v{binding.version}"
        ),
        actor=ACTOR,
    ...
    if readiness.readiness is CapabilityReadiness.AVAILABLE:
        if binding.status == "provisioning":
            binding, _ = registry.update_binding(
                ...
                idempotency_key=(
                    f"R08-{logic_id}-skill-binding-activate-r2-v{binding.version}"
                ),
                actor=ACTOR,
                occurred_at=evaluated_at,
            )
...
def apply(*, now: datetime | None = None) -> dict[str, Any]:
    ...
    if instance.status.value != "active":
        raise BindingCompositionBlocked("SHOPPING_ADVISOR_INSTANCE_NOT_ACTIVE")
    policy = AipModelRuntimeStore().get_policy(SCOPE, POLICY_ID, POLICY_REVISION)
    registry = AipSkillRegistry()
    g04_before = _snapshot_g04(registry)
    results = [
        _ensure_binding(
            registry,
            logic_id=logic_id,
    ...
    g04_after = _snapshot_g04(registry)
    if g04_after != g04_before:
        raise BindingCompositionBlocked("IMMUTABLE_G04_BINDING_CHANGED")
    ...
    return {
        "status": (
            "R08_SHOPPING_ADVISOR_BINDING_GREEN"
            if active_count == len(results)
            else "R08_SHOPPING_ADVISOR_BINDING_GREEN_WITH_RUNTIME_TAIL"
        ),
        "scope": {"orgId": SCOPE.org_id, "projectId": SCOPE.project_id},
