#!/usr/bin/env python3
"""Plan or persist the missing R09 customer-service Logic authority chain.

The default invocation is read-only. ``--apply`` writes only S01/S02/S03/S05/S06
...
creates a Binding, creates an AgentRun or executes customer-service external action.  S04 is
...
ACTOR = "aip-r09-customer-service-bootstrap"
...
def _isolated_registry(model_alias: str) -> RuntimeAdapterRegistry:
    ...
    def invoke(prompt, context):
        ...
        if "__simulate_adapter_error__" in prompt:
            raise LogicAdapterError(
                "LLM_ADAPTER_FAILED", "isolated customer-service adapter failure"
            )
        return LLMAdapterResult(
            output="isolated customer-service draft; not Provider quality evidence",
    ...
    registry.register_llm(
        model_alias,
        invoke,
        adapter_name="r09-customer-service-isolated-contract-eval",
        read_only=True,
        dry_run_safe=True,
    )
    return registry
...
def _get_or_create_graph(logic_id: str, model_alias: str):
    ...
    request = build_customer_service_graph_request(
...
def apply() -> dict[str, Any]:
    ...
    if any(value for counts in before_canary.values() for value in counts.values()):
        raise RuntimeError("negative tenant canary already contains R09 authority")
    s04_before = _s04_snapshot()
    ...
    s04_after = _s04_snapshot()
    if s04_after != s04_before:
        raise RuntimeError("immutable S04 authority changed during R09 bootstrap")
    after_canary = _negative_canary_counts()
    if after_canary != before_canary:
        raise RuntimeError("R09 authority leaked into the negative tenant canary")
    return {
        "status": "R09_CUSTOMER_SERVICE_LOGIC_AUTHORITY_GREEN",
        "scope": {"orgId": SCOPE.org_id, "projectId": SCOPE.project_id},
        "model": {
