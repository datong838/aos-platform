#!/usr/bin/env python3
"""Compose five missing R09 customer-service SkillBindings without AgentRun.

The command is fail-closed. It binds only exact published Skill revisions,
preserves the existing S04 binding and active customer-service instance, and
never calls a Provider or delivers a customer-visible action. Expired capability readiness
...
def binding_id(logic_id: str) -> str:
    return f"ecommerce.customer_service.skill.{logic_id}.r2"
...
def build_plan() -> dict[str, Any]:
    return {
        ...
        "preservedAuthority": [S04_BINDING_ID],
        "bindings": [
            {
                "logicId": logic_id,
...
def _snapshot_s04(registry: AipSkillRegistry) -> dict[str, Any]:
    item = registry.get_binding(SCOPE, S04_BINDING_ID)
    ...
    *,
    ...
    try:
        ...
        binding, _ = registry.create_binding(
            ...
            idempotency_key=f"R09-{logic_id}-skill-binding-create-r2",
            actor=ACTOR,
    ...
    binding, readiness, _ = registry.evaluate_binding(
        ...
        idempotency_key=(
            f"R09-{logic_id}-skill-binding-evaluate-r2-v{binding.version}"
        ),
        actor=ACTOR,
    ...
    if readiness.readiness is CapabilityReadiness.AVAILABLE:
        if binding.status == "provisioning":
            binding, _ = registry.update_binding(
                ...
                idempotency_key=(
                    f"R09-{logic_id}-skill-binding-activate-r2-v{binding.version}"
                ),
                actor=ACTOR,
                occurred_at=evaluated_at,
            )
...
def apply(*, now: datetime | None = None) -> dict[str, Any]:
    ...
    if instance.status.value != "active":
        raise BindingCompositionBlocked("CUSTOMER_SERVICE_INSTANCE_NOT_ACTIVE")
    policy = AipModelRuntimeStore().get_policy(SCOPE, POLICY_ID, POLICY_REVISION)
    registry = AipSkillRegistry()
    s04_before = _snapshot_s04(registry)
    results = [
        _ensure_binding(
            registry,
            logic_id=logic_id,
    ...
    s04_after = _snapshot_s04(registry)
    if s04_after != s04_before:
        raise BindingCompositionBlocked("IMMUTABLE_S04_BINDING_CHANGED")
    ...
    return {
        "status": (
            "R09_CUSTOMER_SERVICE_BINDING_GREEN"
            if active_count == len(results)
            else "R09_CUSTOMER_SERVICE_BINDING_GREEN_WITH_RUNTIME_TAIL"
        ),
        "scope": {"orgId": SCOPE.org_id, "projectId": SCOPE.project_id},
