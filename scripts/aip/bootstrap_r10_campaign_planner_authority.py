#!/usr/bin/env python3
"""Plan or persist the missing R10 campaign-planner Logic authority chain.

The default invocation is read-only. ``--apply`` writes only A01/A03/A04/A05/A06
...
creates a Binding, creates an AgentRun or executes campaign-planner external action.  A02 is
immutable pre-existing authority and is snapshotted before and after.
"""
...
ACTOR = "aip-r10-campaign-planner-bootstrap"
...
def build_plan() -> dict[str, object]:
    return {
        ...
        "preservedAuthority": [A02_GRAPH_ID],
        "steps": [
            f"require exact schema head {REQUIRED_ALEMBIC_HEAD}",
            "read active exact text model alias without resolving Secret payload",
            "snapshot immutable A02 Graph and publications",
            "persist five missing draft-only Graph revisions",
            "run logic-specific isolated governance cases per Logic",
    ...
    def invoke(prompt, context):
        ...
        if "__simulate_adapter_error__" in prompt:
            raise LogicAdapterError(
                "LLM_ADAPTER_FAILED", "isolated campaign-planner adapter failure"
            )
        return LLMAdapterResult(
            output="isolated campaign-planner draft; not Provider quality evidence",
...
def _get_or_create_graph(logic_id: str, model_alias: str):
    ...
    request = build_campaign_planner_graph_request(
...
def _persist_eval(logic_id: str, graph, registry) -> tuple[EvalReportEvidence, str]:
    ...
    if persisted_suite.model_dump(mode="json") != evaluated.suite.model_dump(
        mode="json"
    ):
...
def apply() -> dict[str, Any]:
    ...
    if any(value for counts in before_canary.values() for value in counts.values()):
        raise RuntimeError("negative tenant canary already contains R10 authority")
    a02_before = _a02_snapshot()
    ...
    a02_after = _a02_snapshot()
    if a02_after != a02_before:
        raise RuntimeError("immutable A02 authority changed during R10 bootstrap")
    after_canary = _negative_canary_counts()
    if after_canary != before_canary:
        raise RuntimeError("R10 authority leaked into the negative tenant canary")
    return {
        "status": "R10_CAMPAIGN_PLANNER_LOGIC_AUTHORITY_GREEN",
        "scope": {"orgId": SCOPE.org_id, "projectId": SCOPE.project_id},
        "model": {
