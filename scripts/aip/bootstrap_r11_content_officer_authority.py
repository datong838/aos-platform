#!/usr/bin/env python3
"""Plan or persist the missing R11 content-officer Logic authority chain.

The default invocation is read-only. ``--apply`` writes only C01/C03/C04/C05/C06/C07/C08
...
creates a Binding, creates an AgentRun or executes content-officer external action.  C02 is
...
from aos_api.aip_content_officer_logic import (  # noqa: E402
...
ACTOR = "aip-r11-content-officer-bootstrap"
...
def build_plan() -> dict[str, object]:
    return {
        ...
        "forbiddenSideEffects": [
            ...
            "SkillBinding",
            "AgentRun",
            "content-officer external action",
            "production Action",
            "content publication",
            "media generation or rendering",
            "platform contact or live push",
        ],
    }
    ...
    def invoke(prompt, context):
        ...
        if "__simulate_adapter_error__" in prompt:
            raise LogicAdapterError(
                "LLM_ADAPTER_FAILED", "isolated content-officer adapter failure"
            )
        return LLMAdapterResult(
            output="isolated content-officer draft; not Provider quality evidence",
...
def apply() -> dict[str, Any]:
    ...
    if any(value for counts in before_canary.values() for value in counts.values()):
        raise RuntimeError("negative tenant canary already contains R11 authority")
    c02_before = _c02_snapshot()
    ...
    c02_after = _c02_snapshot()
    if c02_after != c02_before:
        raise RuntimeError("immutable C02 authority changed during R11 bootstrap")
    after_canary = _negative_canary_counts()
    if after_canary != before_canary:
        raise RuntimeError("R11 authority leaked into the negative tenant canary")
    return {
        "status": "R11_CONTENT_OFFICER_LOGIC_AUTHORITY_GREEN",
        "scope": {"orgId": SCOPE.org_id, "projectId": SCOPE.project_id},
        "model": {
