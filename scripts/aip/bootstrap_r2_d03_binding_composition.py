#!/usr/bin/env python3
"""Compose the one approved D03 runnable binding chain without calling Provider.
...
ACTOR = "aip-r2-d03-binding-bootstrap"
...
EVAL_SUITE_ID = "ecommerce.skill.D03.contract.v1"
...
def load_exact_composition(*, evaluated_at: datetime) -> ExactComposition:
    ...
    route = runtime.get_route(SCOPE, ROUTE_ID, 1)
...
def apply(*, now: datetime | None = None) -> dict[str, Any]:
    ...
    return {
        "status": "D03_BINDING_COMPOSITION_GREEN",
        "scope": {"orgId": SCOPE.org_id, "projectId": SCOPE.project_id},
        "capabilityBinding": {
