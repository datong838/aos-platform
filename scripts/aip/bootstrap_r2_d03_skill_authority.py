#!/usr/bin/env python3
"""Publish the exact D03 Skill/Capability control chain for the R2 pilot.
...
from aos_api.aip_d03_pilot import D03_GRAPH_ID, evaluate_d03_contract
...
ACTOR = "aip-r2-d03-skill-bootstrap"
APPROVAL_REF = "46-R2-D03-APPROVED"
...
SUITE_ID = "ecommerce.skill.D03.contract.v1"
DATASET_ID = "ecommerce.skill.D03.contract-cases.v1"
TASK_KEY = "r2-d03-governance-task-v1"
REQUIRED_ALEMBIC_HEAD = "aip10_005"
...
def build_plan() -> dict[str, Any]:
    return {
        ...
        "steps": [
            ...
            "freeze D03 EvalContract",
            "publish immutable D03 Skill r2 and strategy.plan r2",
...
def _isolated_registry(model_alias: str) -> RuntimeAdapterRegistry:
    ...
    def invoke(prompt, context):
        ...
        if "__simulate_adapter_error__" in prompt:
            raise LogicAdapterError("LLM_ADAPTER_FAILED", "isolated D03 adapter failure")
        return LLMAdapterResult(
            output="isolated D03 contract draft; not Provider quality evidence",
    ...
    if plan_row is None:
        plan = tasks.create_plan(
            ...
            CreatePlanRevisionRequest(
                ...
                steps=[PlanStep(step_key="approve-d03", title="核验并记录 D03 受限发布范围")],
    ...
    if run.status.value == "running":
        ...
        if step is None or step["status"] != "succeeded":
            ...
            for phase, payload in (
                ("think", {"approvalRef": APPROVAL_REF, "decision": "restricted_d03_only"}),
                ("act", {"providerCalls": 0, "externalActions": False, "tenant": SCOPE.org_id}),
                ("verify", {"logicId": D03_GRAPH_ID, "skillId": SKILL_ID, "scopeExact": True}),
...
def _skill_eval(source_skill: Any, graph: Any, model: Any):
    evaluated = evaluate_d03_contract(graph, _isolated_registry(_model_alias(model)))
    actual = {item.case_id: item.actual for item in evaluated.report.results}
    expected = {
        "d03-positive": {
            "status": "succeeded",
            "output_contract": "GrowthPlan.DRAFT",
            "usage_present": True,
            "production_written": False,
        },
        "d03-structure": {"blocked": True, "code": "D03_INPUT_STRUCTURE_INVALID"},
        "d03-fact-boundary": {"blocked": True, "code": "D03_FACT_EVIDENCE_REQUIRED"},
        "d03-external-action": {"blocked": True, "code": "D03_EXTERNAL_ACTION_DENIED"},
        "d03-sensitive": {"blocked": True, "code": "D03_SENSITIVE_INPUT_DENIED"},
        "d03-adapter-failure": {
            "status": "failed",
    ...
    if actual != expected:
        raise RuntimeError("isolated D03 contract results drifted before Skill Eval")
    ...
    dataset = DatasetRevisionRef(
        ...
        redaction_policy=AssetRevisionRef(
            ...
            asset_id="d03-no-pii-contract-policy",
            revision="1",
            content_hash=canonical_hash("d03-no-pii-contract-policy-v1"),
        ),
    )
    ...
    if report_row:
        ...
        raise RuntimeError("existing D03 Skill Eval is not terminal GREEN")
    else:
        def resolve(ref: ArtifactRef) -> ResolvedArtifact:
            value = artifacts.get((ref.artifact_id, ref.revision or ""))
            if value is None:
                raise RuntimeError("D03 Eval artifact is unavailable")
        ...
        report = runner.run(
            ...
            execute_judge=lambda ref, observed, wanted: JudgeExecution(
                ...
                detail_code="d03_exact_contract_match",
            ),
        )
    if not report.gate_passed or report.passed != 6 or report.total != 6:
        raise RuntimeError("D03 Skill Eval did not pass 6/6")
    return suite, report
...
def _publication_exact_refs(suite: Any, report: Any):
    ...
    publication = release.publish(
        ...
        request=PublishReleaseRequest(
            ...
            reason_hash=canonical_hash({"approvalRef": APPROVAL_REF, "scope": "D03-only"}),
    ...
    if event_row is None:
        raise RuntimeError("D03 publication event exact row is missing")
...
def apply() -> dict[str, Any]:
    ...
    return {
        "status": "D03_SKILL_CAPABILITY_PUBLICATION_GREEN",
        "scope": {"orgId": SCOPE.org_id, "projectId": SCOPE.project_id},
