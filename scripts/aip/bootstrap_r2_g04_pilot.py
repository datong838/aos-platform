#!/usr/bin/env python3
"""Persist the approved G04 Logic/Eval/Publication chain for R2.

Dry-run is the default.  ``--apply`` writes only the G04 graph, its isolated
...
from aos_api.aip_g04_pilot import (
    G04_EVAL_SUITE_ID,
    G04_GRAPH_ID,
    build_g04_graph_request,
    evaluate_g04_contract,
)
...
def _isolated_registry(model_alias: str) -> RuntimeAdapterRegistry:
    ...
    def invoke(prompt, context):
        ...
        if "__simulate_adapter_error__" in prompt:
            raise LogicAdapterError(
                "LLM_ADAPTER_FAILED", "isolated G04 adapter failure"
            )
    ...
    registry.register_llm(
        model_alias,
        invoke,
        adapter_name="g04-isolated-contract-eval",
        read_only=True,
        dry_run_safe=True,
    )
    return registry
...
def _get_or_create_graph(model_alias: str):
    ...
    request = build_g04_graph_request(model_id=model_alias)
    expected = logic_graph_content_payload(request)
    try:
        graph = store.get(SCOPE.org_id, SCOPE.project_id, G04_GRAPH_ID)
    except LogicGraphNotFound:
        try:
            return store.create(SCOPE.org_id, SCOPE.project_id, ACTOR, request)
        except LogicGraphConflict as exc:
            raise RuntimeError("G04 graph was created concurrently") from exc
    ...
    if actual != expected:
        raise RuntimeError("existing G04 graph differs from the approved exact graph")
    return graph
...
def _persist_successful_run(graph, registry, inputs) -> str:
    ...
    start = run_store.start_run(
        ...
        graph,
        request,
        f"logic-run-g04-{uuid.uuid4().hex}",
    )
    if start.replay is not None:
        if start.replay.status != "succeeded":
            raise RuntimeError("existing canonical G04 dry-run did not succeed")
    ...
    if result.status != "succeeded" or result.production_written is not False:
        raise RuntimeError("canonical G04 dry-run failed closed")
    return result.run_id


def _persist_eval(graph, registry) -> tuple[EvalReportEvidence, str]:
    evaluated = evaluate_g04_contract(graph, registry, now=datetime.now(UTC))
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError("G04 Eval suite was created concurrently") from exc
    if persisted_suite.model_dump(mode="json") != evaluated.suite.model_dump(mode="json"):
        raise RuntimeError("persisted G04 Eval suite drifted")
    ...
    if actual_target != expected_target or not report.gate_passed:
        raise RuntimeError("persisted G04 Eval report does not match exact graph")
    positive = next(
        case.inputs for case in evaluated.suite.cases if case.id == "g04-positive"
    )
...
def apply() -> dict[str, Any]:
    ...
    if any(before_canary.values()):
        raise RuntimeError("negative tenant canary already contains G04 authority")
    ...
    if after_canary != before_canary:
        raise RuntimeError("G04 authority leaked into the negative tenant canary")
    return {
        "status": "G04_LOGIC_PUBLICATION_GREEN",
        "scope": {"orgId": SCOPE.org_id, "projectId": SCOPE.project_id},
        "graph": {
            "assetId": graph.id,
