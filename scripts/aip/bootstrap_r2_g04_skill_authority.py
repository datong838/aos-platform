#!/usr/bin/env python3
"""Publish the exact G04 Skill/Capability control chain for the R2 pilot.
...
from aos_api.aip_g04_pilot import G04_GRAPH_ID, evaluate_g04_contract
...
ACTOR = "aip-r2-g04-skill-bootstrap"
APPROVAL_REF = "46-R2-G04-SHOPPING-ADVISOR"
SKILL_ID = "ecommerce.skill.G04"
CAPABILITY_ID = "strategy.plan"
ROUTE_ID = "route-qyh-text-dev"
POLICY_ID = "policy-qyh-text-dev"
MODEL_ID = "model-qyh-text-dev"
SUITE_ID = "ecommerce.skill.G04.contract.v1"
DATASET_ID = "ecommerce.skill.G04.contract-cases.v1"
TASK_KEY = "r2-g04-governance-task-v1"
...
def build_plan() -> dict[str, Any]:
    return {
        ...
        "steps": [
            ...
            "freeze G04 EvalContract",
            "publish immutable G04 Skill r2; reuse strategy.plan r2",
...
def _isolated_registry(model_alias: str) -> RuntimeAdapterRegistry:
    ...
    def invoke(prompt, context):
        ...
        if "__simulate_adapter_error__" in prompt:
            raise LogicAdapterError("LLM_ADAPTER_FAILED", "isolated G04 adapter failure")
        return LLMAdapterResult(
            output="isolated G04 contract draft; not Provider quality evidence",
    ...
    registry.register_llm(
        model_alias,
        invoke,
        adapter_name="g04-isolated-skill-contract-eval",
        read_only=True,
        dry_run_safe=True,
    )
    return registry
...
def _governance_evidence() -> tuple[ExactRevisionRef, ExactRevisionRef]:
    ...
    if plan_row is None:
        plan = tasks.create_plan(
            ...
            CreatePlanRevisionRequest(
                ...
                steps=[PlanStep(step_key="approve-g04", title="核验并记录 G04 受限发布范围")],
    ...
    if run.status.value == "running":
        with db_connect(SCOPE) as conn:
            step = conn.execute(
                "SELECT * FROM aip_step_run WHERE org_id=%s AND project_id=%s AND run_id=%s AND step_key='approve-g04' ORDER BY attempt DESC LIMIT 1",
                (*SCOPE.key, run.id),
            ).fetchone()
        if step is None or step["status"] != "succeeded":
            lease = tasks.claim_step(SCOPE, run.id, "approve-g04", ACTOR, lease_seconds=300)
            for phase, payload in (
                ("think", {"approvalRef": APPROVAL_REF, "decision": "restricted_g04_only"}),
                ("act", {"providerCalls": 0, "externalActions": False, "tenant": SCOPE.org_id}),
                ("verify", {"logicId": G04_GRAPH_ID, "skillId": SKILL_ID, "scopeExact": True}),
    ...
    bundle = contracts.create_evidence_bundle(
        ...
        CreateEvidenceBundleRequest(
            ...
            license_summary={
                ...
                "approvedCapabilities": ["text", "llm", "chat"],
                "forbiddenCapabilities": ["image", "audio", "video", "tool_execution"],
...
def _skill_eval(source_skill: Any, graph: Any, model: Any):
    evaluated = evaluate_g04_contract(graph, _isolated_registry(_model_alias(model)))
    actual = {item.case_id: item.actual for item in evaluated.report.results}
    expected = {
        "g04-positive": {
            "status": "succeeded",
            "output_contract": "RecommendationDraft.DRAFT",
            "usage_present": True,
            "production_written": False,
        },
        "g04-structure": {"blocked": True, "code": "G04_INPUT_STRUCTURE_INVALID"},
        "g04-fact-boundary": {"blocked": True, "code": "G04_FACT_EVIDENCE_REQUIRED"},
        "g04-external-action": {"blocked": True, "code": "G04_EXTERNAL_ACTION_DENIED"},
        "g04-sensitive": {"blocked": True, "code": "G04_SENSITIVE_INPUT_DENIED"},
        "g04-adapter-failure": {
            "status": "failed",
    ...
    if actual != expected:
        raise RuntimeError("isolated G04 contract results drifted before Skill Eval")
    artifacts: dict[tuple[str, str], Any] = {}
    cases: list[EvalCaseDefinition] = []
    kinds = {
        "g04-positive": EvalCaseKind.POSITIVE,
        "g04-structure": EvalCaseKind.NEGATIVE,
        "g04-fact-boundary": EvalCaseKind.BOUNDARY,
        "g04-external-action": EvalCaseKind.NEGATIVE,
        "g04-sensitive": EvalCaseKind.PII,
        "g04-adapter-failure": EvalCaseKind.TOOL_FAILURE,
    }
    ...
    dataset = DatasetRevisionRef(
        ...
        redaction_policy=AssetRevisionRef(
            ...
            asset_id="g04-no-pii-contract-policy",
            revision="1",
            content_hash=canonical_hash("g04-no-pii-contract-policy-v1"),
        ),
    )
    manifest = EvalDatasetManifest(
        source_kind=DatasetSourceKind.ARTIFACT_SNAPSHOT,
        source_id="g04-isolated-contract-cases",
        source_revision="1",
        source_hash=source_hash,
        fields_allowlist=["caseId"],
        redaction_receipt=ArtifactRef(
            artifact_id="g04-contract-redaction-receipt",
            artifact_type="receipt",
            revision="1",
            content_hash=canonical_hash("g04-no-pii-reviewed"),
        ),
    ...
    judge = JudgeRevisionRef(
        judge_id="g04-exact-contract-judge",
        revision=1,
        content_hash=canonical_hash("g04-exact-contract-judge-v1"),
    )
    ...
    if report_row:
        ...
        raise RuntimeError("existing G04 Skill Eval is not terminal GREEN")
    else:
        def resolve(ref: ArtifactRef) -> ResolvedArtifact:
            value = artifacts.get((ref.artifact_id, ref.revision or ""))
            if value is None:
                raise RuntimeError("G04 Eval artifact is unavailable")
        ...
        report = runner.run(
            ...
            execute_judge=lambda ref, observed, wanted: JudgeExecution(
                ...
                detail_code="g04_exact_contract_match",
            ),
        )
    if not report.gate_passed or report.passed != 6 or report.total != 6:
        raise RuntimeError("G04 Skill Eval did not pass 6/6")
    return suite, report
...
def _publication_exact_refs(suite: Any, report: Any):
    ...
    publication = release.publish(
        ...
        request=PublishReleaseRequest(
            ...
            reason_hash=canonical_hash({"approvalRef": APPROVAL_REF, "scope": "G04-only"}),
    ...
    if event_row is None:
        raise RuntimeError("G04 publication event exact row is missing")
    ...
    contract = store.create_eval_contract(
        ...
        CreateEvalContractRequest(
            ...
            artifact_schema_ref=ResourceRef(
                resource_type="Schema",
                resource_id="RecommendationDraft.DRAFT",
                revision="1",
                authority="aip-contract",
            ),
...
def _publish_skill_and_capability(
    ...
    # Reuse existing strategy.plan r2; do not publish a new capability revision.
    capability = AipCapabilityRegistry().get(CAPABILITY_ID, 2)
...
def apply() -> dict[str, Any]:
    ...
    graph = LogicGraphStore().get(SCOPE.org_id, SCOPE.project_id, G04_GRAPH_ID)
    model_store = AipModelRuntimeStore()
    model = model_store.get_model(SCOPE, MODEL_ID, 3)
    route = model_store.get_route(SCOPE, ROUTE_ID, 4)
    policy = model_store.get_policy(SCOPE, POLICY_ID, 2)
    source_skill = AipSkillRegistry().get_skill(SKILL_ID, 1)
    ...
    return {
        "status": "G04_SKILL_CAPABILITY_PUBLICATION_GREEN",
        "scope": {"orgId": SCOPE.org_id, "projectId": SCOPE.project_id},
