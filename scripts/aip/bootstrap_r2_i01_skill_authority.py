#!/usr/bin/env python3
"""Publish the exact I01 Skill/Capability control chain for the R2 pilot.
...
from aos_api.aip_i01_pilot import I01_GRAPH_ID, evaluate_i01_contract
...
ACTOR = "aip-r2-i01-skill-bootstrap"
APPROVAL_REF = "46-R2-I01-VISUAL-DRAFT"
SKILL_ID = "ecommerce.skill.I01"
CAPABILITY_ID = "image.generate"
ROUTE_ID = "route-qyh-image-dev"
POLICY_ID = "policy-qyh-image-dev"
MODEL_ID = "model-qyh-image-dev"
SUITE_ID = "ecommerce.skill.I01.contract.v1"
DATASET_ID = "ecommerce.skill.I01.contract-cases.v1"
TASK_KEY = "r2-i01-governance-task-v1"
...
def build_plan() -> dict[str, Any]:
    return {
        ...
        "steps": [
            ...
            "freeze I01 EvalContract",
            "ensure image.generate r1 and I01 Skill r1 exist",
            "publish immutable I01 Skill r2 and image.generate r2",
...
def _isolated_registry(model_alias: str) -> RuntimeAdapterRegistry:
    ...
    def invoke(prompt, context):
        ...
        if "__simulate_adapter_error__" in prompt:
            raise LogicAdapterError("LLM_ADAPTER_FAILED", "isolated I01 adapter failure")
        return LLMAdapterResult(
            output="isolated I01 contract draft; not Provider quality evidence",
    ...
    registry.register_llm(
        model_alias,
        invoke,
        adapter_name="i01-isolated-skill-contract-eval",
        read_only=True,
        dry_run_safe=True,
    )
    return registry
    ...
    if plan_row is None:
        plan = tasks.create_plan(
            ...
            CreatePlanRevisionRequest(
                ...
                steps=[PlanStep(step_key="approve-i01", title="核验并记录 I01 受限发布范围")],
    ...
    if run.status.value == "running":
        ...
        if step is None or step["status"] != "succeeded":
            ...
            for phase, payload in (
                ("think", {"approvalRef": APPROVAL_REF, "decision": "restricted_i01_only"}),
                ("act", {"providerCalls": 0, "externalActions": False, "tenant": SCOPE.org_id}),
                ("verify", {"logicId": I01_GRAPH_ID, "skillId": SKILL_ID, "scopeExact": True}),
    ...
    bundle = contracts.create_evidence_bundle(
        ...
        CreateEvidenceBundleRequest(
            ...
            license_summary={
                ...
                "approvedCapabilities": ["text", "image", "llm"],
                "forbiddenCapabilities": ["audio", "video", "tool_execution"],
...
def _skill_eval(source_skill: Any, graph: Any, model: Any):
    ...
    expected = {
        "i01-positive": {
            "status": "succeeded",
            "output_contract": "VisualDraft.DRAFT",
            "usage_present": True,
            "production_written": False,
        },
        "i01-structure": {"blocked": True, "code": "I01_INPUT_STRUCTURE_INVALID"},
        "i01-fact-boundary": {"blocked": True, "code": "I01_FACT_EVIDENCE_REQUIRED"},
        "i01-external-action": {"blocked": True, "code": "I01_EXTERNAL_ACTION_DENIED"},
        "i01-sensitive": {"blocked": True, "code": "I01_SENSITIVE_INPUT_DENIED"},
        "i01-adapter-failure": {
            "status": "failed",
    ...
    if actual != expected:
        raise RuntimeError("isolated I01 contract results drifted before Skill Eval")
    artifacts: dict[tuple[str, str], Any] = {}
    cases: list[EvalCaseDefinition] = []
    kinds = {
        "i01-positive": EvalCaseKind.POSITIVE,
        "i01-structure": EvalCaseKind.NEGATIVE,
        "i01-fact-boundary": EvalCaseKind.BOUNDARY,
        "i01-external-action": EvalCaseKind.NEGATIVE,
        "i01-sensitive": EvalCaseKind.PII,
        "i01-adapter-failure": EvalCaseKind.TOOL_FAILURE,
    }
    ...
    dataset = DatasetRevisionRef(
        ...
        redaction_policy=AssetRevisionRef(
            ...
            asset_id="i01-no-pii-contract-policy",
            revision="1",
            content_hash=canonical_hash("i01-no-pii-contract-policy-v1"),
        ),
    )
    manifest = EvalDatasetManifest(
        source_kind=DatasetSourceKind.ARTIFACT_SNAPSHOT,
        source_id="i01-isolated-contract-cases",
        source_revision="1",
        source_hash=source_hash,
        fields_allowlist=["caseId"],
        redaction_receipt=ArtifactRef(
            artifact_id="i01-contract-redaction-receipt",
            artifact_type="receipt",
            revision="1",
            content_hash=canonical_hash("i01-no-pii-reviewed"),
        ),
    ...
    judge = JudgeRevisionRef(
        judge_id="i01-exact-contract-judge",
        revision=1,
        content_hash=canonical_hash("i01-exact-contract-judge-v1"),
    )
    ...
    if report_row:
        ...
        raise RuntimeError("existing I01 Skill Eval is not terminal GREEN")
    else:
        def resolve(ref: ArtifactRef) -> ResolvedArtifact:
            value = artifacts.get((ref.artifact_id, ref.revision or ""))
            if value is None:
                raise RuntimeError("I01 Eval artifact is unavailable")
    ...
    if not report.gate_passed or report.passed != 6 or report.total != 6:
        raise RuntimeError("I01 Skill Eval did not pass 6/6")
    return suite, report
...
def _publication_exact_refs(suite: Any, report: Any):
    ...
    publication = release.publish(
        ...
        request=PublishReleaseRequest(
            ...
            reason_hash=canonical_hash({"approvalRef": APPROVAL_REF, "scope": "I01-only"}),
    ...
    if event_row is None:
        raise RuntimeError("I01 publication event exact row is missing")
...
def _ensure_capability_r1() -> Any:
    ...
    payload = {
        ...
        "parentRef": None,
        "aliases": ["visual.image.generate", "content.image.generate"],
        "inputSchemaRef": _schema_ref("ImageGenerationRequest").model_dump(
            mode="json", by_alias=True
        ),
        "outputSchemaRef": _schema_ref("ImageDraftRef").model_dump(
            mode="json", by_alias=True
        ),
        "riskLevel": "medium",
        "requiredDataRefs": [],
...
def apply() -> dict[str, Any]:
    ...
    graph = LogicGraphStore().get(SCOPE.org_id, SCOPE.project_id, I01_GRAPH_ID)
    ...
    return {
        "status": "I01_SKILL_CAPABILITY_PUBLICATION_GREEN",
        "scope": {"orgId": SCOPE.org_id, "projectId": SCOPE.project_id},
