#!/usr/bin/env python3
"""Compose the one approved P02 runnable binding chain without calling Provider.
...
ACTOR = "aip-r2-p02-binding-bootstrap"
...
EVAL_SUITE_ID = "ecommerce.skill.P02.contract.v1"
...
def apply(*, now: datetime | None = None) -> dict[str, Any]:
    ...
    # Reuse tenant-level strategy.plan binding created for data advisor.
    capability_binding = capability_service.get(SCOPE, CAPABILITY_BINDING_ID)
    capability_binding, readiness, _ = capability_service.evaluate(
        SCOPE,
        CAPABILITY_BINDING_ID,
        EvaluateOperationalBindingRequest(
            expected_version=capability_binding.version,
            dependencies=composition.dependencies,
    ...
    return {
        "status": "P02_BINDING_COMPOSITION_GREEN",
        "scope": {"orgId": SCOPE.org_id, "projectId": SCOPE.project_id},
        "capabilityBinding": {
