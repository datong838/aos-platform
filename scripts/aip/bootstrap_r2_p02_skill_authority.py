#!/usr/bin/env python3
"""Publish the exact P02 Skill/Capability control chain for the R2 pilot.
...
from aos_api.aip_p02_pilot import P02_GRAPH_ID, evaluate_p02_contract
...
ACTOR = "aip-r2-p02-skill-bootstrap"
APPROVAL_REF = "46-R2-P02-PRIVATE-DOMAIN"
SKILL_ID = "ecommerce.skill.P02"
CAPABILITY_ID = "strategy.plan"
ROUTE_ID = "route-qyh-text-dev"
POLICY_ID = "policy-qyh-text-dev"
MODEL_ID = "model-qyh-text-dev"
SUITE_ID = "ecommerce.skill.P02.contract.v1"
DATASET_ID = "ecommerce.skill.P02.contract-cases.v1"
TASK_KEY = "r2-p02-governance-task-v1"
...
def build_plan() -> dict[str, Any]:
    return {
        ...
        "steps": [
            ...
            "freeze P02 EvalContract",
            "publish immutable P02 Skill r2; reuse strategy.plan r2",
...
def _isolated_registry(model_alias: str) -> RuntimeAdapterRegistry:
    ...
    def invoke(prompt, context):
        ...
        if "__simulate_adapter_error__" in prompt:
            raise LogicAdapterError("LLM_ADAPTER_FAILED", "isolated P02 adapter failure")
        return LLMAdapterResult(
            output="isolated P02 contract draft; not Provider quality evidence",
    ...
    registry.register_llm(
        model_alias,
        invoke,
        adapter_name="p02-isolated-skill-contract-eval",
        read_only=True,
        dry_run_safe=True,
    )
    return registry
...
def _governance_evidence() -> tuple[ExactRevisionRef, ExactRevisionRef]:
    ...
    if plan_row is None:
        plan = tasks.create_plan(
            ...
            CreatePlanRevisionRequest(
                ...
                steps=[PlanStep(step_key="approve-p02", title="核验并记录 P02 受限发布范围")],
    ...
    if run.status.value == "running":
        with db_connect(SCOPE) as conn:
            step = conn.execute(
                "SELECT * FROM aip_step_run WHERE org_id=%s AND project_id=%s AND run_id=%s AND step_key='approve-p02' ORDER BY attempt DESC LIMIT 1",
                (*SCOPE.key, run.id),
            ).fetchone()
        if step is None or step["status"] != "succeeded":
            lease = tasks.claim_step(SCOPE, run.id, "approve-p02", ACTOR, lease_seconds=300)
            for phase, payload in (
                ("think", {"approvalRef": APPROVAL_REF, "decision": "restricted_p02_only"}),
                ("act", {"providerCalls": 0, "externalActions": False, "tenant": SCOPE.org_id}),
                ("verify", {"logicId": P02_GRAPH_ID, "skillId": SKILL_ID, "scopeExact": True}),
...
def _skill_eval(source_skill: Any, graph: Any, model: Any):
    evaluated = evaluate_p02_contract(graph, _isolated_registry(_model_alias(model)))
    actual = {item.case_id: item.actual for item in evaluated.report.results}
    expected = {
        "p02-positive": {
            "status": "succeeded",
            "output_contract": "PrivateDomainPlan.DRAFT",
            "usage_present": True,
            "production_written": False,
        },
        "p02-structure": {"blocked": True, "code": "P02_INPUT_STRUCTURE_INVALID"},
        "p02-fact-boundary": {"blocked": True, "code": "P02_FACT_EVIDENCE_REQUIRED"},
        "p02-external-action": {"blocked": True, "code": "P02_EXTERNAL_ACTION_DENIED"},
        "p02-sensitive": {"blocked": True, "code": "P02_SENSITIVE_INPUT_DENIED"},
        "p02-adapter-failure": {
            "status": "failed",
    ...
    if actual != expected:
        raise RuntimeError("isolated P02 contract results drifted before Skill Eval")
    artifacts: dict[tuple[str, str], Any] = {}
    cases: list[EvalCaseDefinition] = []
    kinds = {
        "p02-positive": EvalCaseKind.POSITIVE,
        "p02-structure": EvalCaseKind.NEGATIVE,
        "p02-fact-boundary": EvalCaseKind.BOUNDARY,
        "p02-external-action": EvalCaseKind.NEGATIVE,
        "p02-sensitive": EvalCaseKind.PII,
        "p02-adapter-failure": EvalCaseKind.TOOL_FAILURE,
    }
    ...
    dataset = DatasetRevisionRef(
        ...
        redaction_policy=AssetRevisionRef(
            ...
            asset_id="p02-no-pii-contract-policy",
            revision="1",
            content_hash=canonical_hash("p02-no-pii-contract-policy-v1"),
        ),
    )
    manifest = EvalDatasetManifest(
        source_kind=DatasetSourceKind.ARTIFACT_SNAPSHOT,
        source_id="p02-isolated-contract-cases",
        source_revision="1",
        source_hash=source_hash,
        fields_allowlist=["caseId"],
        redaction_receipt=ArtifactRef(
            artifact_id="p02-contract-redaction-receipt",
            artifact_type="receipt",
            revision="1",
            content_hash=canonical_hash("p02-no-pii-reviewed"),
        ),
    ...
    judge = JudgeRevisionRef(
        judge_id="p02-exact-contract-judge",
        revision=1,
        content_hash=canonical_hash("p02-exact-contract-judge-v1"),
    )
    ...
    if report_row:
        ...
        raise RuntimeError("existing P02 Skill Eval is not terminal GREEN")
    else:
        def resolve(ref: ArtifactRef) -> ResolvedArtifact:
            value = artifacts.get((ref.artifact_id, ref.revision or ""))
            if value is None:
                raise RuntimeError("P02 Eval artifact is unavailable")
        ...
        report = runner.run(
            ...
            execute_judge=lambda ref, observed, wanted: JudgeExecution(
                ...
                detail_code="p02_exact_contract_match",
            ),
        )
    if not report.gate_passed or report.passed != 6 or report.total != 6:
        raise RuntimeError("P02 Skill Eval did not pass 6/6")
    return suite, report
...
def _publication_exact_refs(suite: Any, report: Any):
    ...
    publication = release.publish(
        ...
        request=PublishReleaseRequest(
            ...
            reason_hash=canonical_hash({"approvalRef": APPROVAL_REF, "scope": "P02-only"}),
    ...
    if event_row is None:
        raise RuntimeError("P02 publication event exact row is missing")
    ...
    contract = store.create_eval_contract(
        ...
        CreateEvalContractRequest(
            ...
            artifact_schema_ref=ResourceRef(
                resource_type="Schema",
                resource_id="PrivateDomainPlan.DRAFT",
                revision="1",
                authority="aip-contract",
            ),
...
def apply() -> dict[str, Any]:
    ...
    graph = LogicGraphStore().get(SCOPE.org_id, SCOPE.project_id, P02_GRAPH_ID)
    ...
    return {
        "status": "P02_SKILL_CAPABILITY_PUBLICATION_GREEN",
        "scope": {"orgId": SCOPE.org_id, "projectId": SCOPE.project_id},
