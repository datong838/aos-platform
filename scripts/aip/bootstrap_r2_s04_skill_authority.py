#!/usr/bin/env python3
"""Publish the exact S04 Skill/Capability control chain for the R2 pilot.
...
from aos_api.aip_s04_pilot import S04_GRAPH_ID, evaluate_s04_contract
...
ACTOR = "aip-r2-s04-skill-bootstrap"
APPROVAL_REF = "46-R2-S04-CUSTOMER-SERVICE"
SKILL_ID = "ecommerce.skill.S04"
CAPABILITY_ID = "strategy.plan"
ROUTE_ID = "route-qyh-text-dev"
POLICY_ID = "policy-qyh-text-dev"
MODEL_ID = "model-qyh-text-dev"
SUITE_ID = "ecommerce.skill.S04.contract.v1"
DATASET_ID = "ecommerce.skill.S04.contract-cases.v1"
TASK_KEY = "r2-s04-governance-task-v1"
...
def build_plan() -> dict[str, Any]:
    return {
        ...
        "steps": [
            ...
            "freeze S04 EvalContract",
            "publish immutable S04 Skill r2; reuse strategy.plan r2",
...
def _isolated_registry(model_alias: str) -> RuntimeAdapterRegistry:
    ...
    def invoke(prompt, context):
        ...
        if "__simulate_adapter_error__" in prompt:
            raise LogicAdapterError("LLM_ADAPTER_FAILED", "isolated S04 adapter failure")
        return LLMAdapterResult(
            output="isolated S04 contract draft; not Provider quality evidence",
    ...
    registry.register_llm(
        model_alias,
        invoke,
        adapter_name="s04-isolated-skill-contract-eval",
        read_only=True,
        dry_run_safe=True,
    )
    return registry
...
def _governance_evidence() -> tuple[ExactRevisionRef, ExactRevisionRef]:
    ...
    if plan_row is None:
        plan = tasks.create_plan(
            ...
            CreatePlanRevisionRequest(
                ...
                steps=[PlanStep(step_key="approve-s04", title="核验并记录 S04 受限发布范围")],
    ...
    if run.status.value == "running":
        with db_connect(SCOPE) as conn:
            step = conn.execute(
                "SELECT * FROM aip_step_run WHERE org_id=%s AND project_id=%s AND run_id=%s AND step_key='approve-s04' ORDER BY attempt DESC LIMIT 1",
                (*SCOPE.key, run.id),
            ).fetchone()
        if step is None or step["status"] != "succeeded":
            lease = tasks.claim_step(SCOPE, run.id, "approve-s04", ACTOR, lease_seconds=300)
            for phase, payload in (
                ("think", {"approvalRef": APPROVAL_REF, "decision": "restricted_s04_only"}),
...
def _skill_eval(source_skill: Any, graph: Any, model: Any):
    evaluated = evaluate_s04_contract(graph, _isolated_registry(_model_alias(model)))
    actual = {item.case_id: item.actual for item in evaluated.report.results}
    expected = {
        "s04-positive": {
            "status": "succeeded",
            "output_contract": "ServiceAction.DRAFT",
            "usage_present": True,
            "production_written": False,
        },
        "s04-structure": {"blocked": True, "code": "S04_INPUT_STRUCTURE_INVALID"},
        "s04-fact-boundary": {"blocked": True, "code": "S04_FACT_EVIDENCE_REQUIRED"},
        "s04-external-action": {"blocked": True, "code": "S04_EXTERNAL_ACTION_DENIED"},
        "s04-sensitive": {"blocked": True, "code": "S04_SENSITIVE_INPUT_DENIED"},
        "s04-adapter-failure": {
            "status": "failed",
    ...
    if actual != expected:
        raise RuntimeError("isolated S04 contract results drifted before Skill Eval")
    artifacts: dict[tuple[str, str], Any] = {}
    cases: list[EvalCaseDefinition] = []
    kinds = {
        "s04-positive": EvalCaseKind.POSITIVE,
        "s04-structure": EvalCaseKind.NEGATIVE,
        "s04-fact-boundary": EvalCaseKind.BOUNDARY,
        "s04-external-action": EvalCaseKind.NEGATIVE,
        "s04-sensitive": EvalCaseKind.PII,
        "s04-adapter-failure": EvalCaseKind.TOOL_FAILURE,
    }
    ...
    dataset = DatasetRevisionRef(
        ...
        redaction_policy=AssetRevisionRef(
            ...
            asset_id="s04-no-pii-contract-policy",
            revision="1",
            content_hash=canonical_hash("s04-no-pii-contract-policy-v1"),
        ),
    )
    manifest = EvalDatasetManifest(
        source_kind=DatasetSourceKind.ARTIFACT_SNAPSHOT,
        source_id="s04-isolated-contract-cases",
        source_revision="1",
        source_hash=source_hash,
        fields_allowlist=["caseId"],
        redaction_receipt=ArtifactRef(
            artifact_id="s04-contract-redaction-receipt",
            artifact_type="receipt",
            revision="1",
            content_hash=canonical_hash("s04-no-pii-reviewed"),
        ),
    ...
    judge = JudgeRevisionRef(
        judge_id="s04-exact-contract-judge",
        revision=1,
        content_hash=canonical_hash("s04-exact-contract-judge-v1"),
    )
    ...
    if report_row:
        ...
        raise RuntimeError("existing S04 Skill Eval is not terminal GREEN")
    else:
        def resolve(ref: ArtifactRef) -> ResolvedArtifact:
            value = artifacts.get((ref.artifact_id, ref.revision or ""))
            if value is None:
                raise RuntimeError("S04 Eval artifact is unavailable")
        ...
        report = runner.run(
            ...
            execute_judge=lambda ref, observed, wanted: JudgeExecution(
                ...
                detail_code="s04_exact_contract_match",
            ),
        )
    if not report.gate_passed or report.passed != 6 or report.total != 6:
        raise RuntimeError("S04 Skill Eval did not pass 6/6")
    return suite, report
...
def _publication_exact_refs(suite: Any, report: Any):
    ...
    publication = release.publish(
        ...
        request=PublishReleaseRequest(
            ...
            reason_hash=canonical_hash({"approvalRef": APPROVAL_REF, "scope": "S04-only"}),
    ...
    if event_row is None:
        raise RuntimeError("S04 publication event exact row is missing")
    ...
    contract = store.create_eval_contract(
        ...
        CreateEvalContractRequest(
            ...
            artifact_schema_ref=ResourceRef(
                resource_type="Schema",
                resource_id="ServiceAction.DRAFT",
                revision="1",
                authority="aip-contract",
            ),
...
def apply() -> dict[str, Any]:
    ...
    graph = LogicGraphStore().get(SCOPE.org_id, SCOPE.project_id, S04_GRAPH_ID)
    ...
    return {
        "status": "S04_SKILL_CAPABILITY_PUBLICATION_GREEN",
        "scope": {"orgId": SCOPE.org_id, "projectId": SCOPE.project_id},
