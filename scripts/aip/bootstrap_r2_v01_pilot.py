#!/usr/bin/env python3
"""Persist the approved V01 Logic/Eval/Publication chain for R2.

Dry-run is the default.  ``--apply`` writes only the V01 graph, its isolated
...
from aos_api.aip_v01_pilot import (
    V01_EVAL_SUITE_ID,
    V01_GRAPH_ID,
...
def _isolated_registry(model_alias: str) -> RuntimeAdapterRegistry:
    ...
    def invoke(prompt, context):
        ...
        if "__simulate_adapter_error__" in prompt:
            raise LogicAdapterError(
                "LLM_ADAPTER_FAILED", "isolated V01 adapter failure"
            )
    ...
    registry.register_llm(
        model_alias,
        invoke,
        adapter_name="v01-isolated-contract-eval",
        read_only=True,
        dry_run_safe=True,
    )
    return registry
...
def _get_or_create_graph(model_alias: str):
    ...
    try:
        graph = store.get(SCOPE.org_id, SCOPE.project_id, V01_GRAPH_ID)
    except LogicGraphNotFound:
        try:
            return store.create(SCOPE.org_id, SCOPE.project_id, ACTOR, request)
        except LogicGraphConflict as exc:
            raise RuntimeError("V01 graph was created concurrently") from exc
    ...
    if actual != expected:
        raise RuntimeError("existing V01 graph differs from the approved exact graph")
    return graph
...
def _persist_successful_run(graph, registry, inputs) -> str:
    ...
    start = run_store.start_run(
        ...
        graph,
        request,
        f"logic-run-v01-{uuid.uuid4().hex}",
    )
    if start.replay is not None:
        if start.replay.status != "succeeded":
            raise RuntimeError("existing canonical V01 dry-run did not succeed")
    ...
    if result.status != "succeeded" or result.production_written is not False:
        raise RuntimeError("canonical V01 dry-run failed closed")
...
def _persist_eval(graph, registry) -> tuple[EvalReportEvidence, str]:
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError("V01 Eval suite was created concurrently") from exc
    if persisted_suite.model_dump(mode="json") != evaluated.suite.model_dump(mode="json"):
        raise RuntimeError("persisted V01 Eval suite drifted")
    ...
    if actual_target != expected_target or not report.gate_passed:
        raise RuntimeError("persisted V01 Eval report does not match exact graph")
    positive = next(
        case.inputs for case in evaluated.suite.cases if case.id == "v01-positive"
    )
...
def apply() -> dict[str, Any]:
    ...
    if any(before_canary.values()):
        raise RuntimeError("negative tenant canary already contains V01 authority")
    ...
    if after_canary != before_canary:
        raise RuntimeError("V01 authority leaked into the negative tenant canary")
    return {
        "status": "V01_LOGIC_PUBLICATION_GREEN",
        "scope": {"orgId": SCOPE.org_id, "projectId": SCOPE.project_id},
        "graph": {
            "assetId": graph.id,
