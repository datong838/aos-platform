#!/usr/bin/env python3
"""Bootstrap Agnes Video Provider authority (VID-1/VID-2).

Default is inspect. ``--apply`` publishes additive:
  Egress + Network + Provider ``agnes-video-qyh-dev``

Secret binding is plugin/provider exact:
  ``keychain://com.aos.llm/agnes-video/org-org/dev-project/agnes-video-qyh-dev#api-key``
...
ACTOR = "aip-r2-vid-provider-bootstrap"
APPROVAL_REF = "46-§8.77-VID-2"
CN_HOST = "api.agnes-ai.cn"
CN_BASE_URL = "https://api.agnes-ai.cn/v1"
CN_REGION = "China (Domestic)"
PLUGIN_ID = "agnes-video"
PROVIDER_ID = "agnes-video-qyh-dev"
EGRESS_ID = "agnes-video-qyh-dev-egress"
NETWORK_ID = "network-qyh-video-dev"
TEXT_PROVIDER_ID = "agnes-text-qyh-dev"
TEXT_PROVIDER_REVISION = 7
VIDEO_SECRET_REF = (
    "keychain://com.aos.llm/agnes-video/org-org/dev-project/"
    "agnes-video-qyh-dev#api-key"
)
VIDEO_SECRET_VERSION = "v1"
...
def _provider_payload(
    *,
    ...
    return {
        ...
        "secretRef": VIDEO_SECRET_REF,
        "secretVersion": VIDEO_SECRET_VERSION,
...
def inspect() -> dict[str, Any]:
    ...
    if provider_head is not None:
        ...
        secret_ok = provider.secret_ref == VIDEO_SECRET_REF
        return {
...
def apply() -> dict[str, Any]:
    ...
    if snapshot["status"] == "already_present":
        return {**snapshot, "status": "R2_VID_PROVIDER_AUTHORITY_GREEN"}
    ...
    return {
        "status": "R2_VID_PROVIDER_AUTHORITY_GREEN",
        "scope": {"orgId": SCOPE.org_id, "projectId": SCOPE.project_id},
        "plugin": exact_ref(
            "ProviderPluginRevision", plugin, "provider_plugin_id"
        ).model_dump(mode="json", by_alias=True),
...
def main() -> int:
    ...
    return 0 if result.get("status") in {
        ...
        "R2_VID_PROVIDER_AUTHORITY_GREEN",
    } else 2
