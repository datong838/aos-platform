#!/usr/bin/env python3
"""Execute the single approved A02 real-provider pilot.
...
SKILL_REVISION = 2
SKILL_BINDING_ID = "ecommerce.campaign_planner.skill.A02.r2"
ROUTE_ID = "route-qyh-text-dev"
CAPABILITY_BINDING_ID = "ecommerce.data_advisor.strategy.plan.r2"
TASK_KEY = "r2-a02-real-pilot-task-v1"
PLAN_KEY = "r2-a02-real-pilot-plan-v1"
TASK_RUN_KEY = "r2-a02-real-pilot-task-run-v1"
AGENT_RUN_ID = "ecommerce.campaign_planner.A02.real-pilot.v1"
ATTEMPT_ID = "ecommerce.campaign_planner.A02.real-pilot.v1.attempt-1"
EXECUTE_KEY = "r2-a02-real-pilot-execute-v1"
STEP_KEY = "execute-a02-pilot"

INCIDENT_AGENT_RUN_ID = "ecommerce.campaign_planner.A02.real-pilot.v7"
INCIDENT_ATTEMPT_ID = "ecommerce.campaign_planner.A02.real-pilot.v7.attempt-1"
PRIOR_INCIDENT_V6_AGENT_RUN_ID = "ecommerce.campaign_planner.A02.real-pilot.v6"
PRIOR_INCIDENT_V6_ATTEMPT_ID = "ecommerce.campaign_planner.A02.real-pilot.v6.attempt-1"
PRIOR_INCIDENT_V5_AGENT_RUN_ID = "ecommerce.campaign_planner.A02.real-pilot.v5"
PRIOR_INCIDENT_V5_ATTEMPT_ID = "ecommerce.campaign_planner.A02.real-pilot.v5.attempt-1"
PRIOR_INCIDENT_V4_AGENT_RUN_ID = "ecommerce.campaign_planner.A02.real-pilot.v4"
PRIOR_INCIDENT_V4_ATTEMPT_ID = "ecommerce.campaign_planner.A02.real-pilot.v4.attempt-1"
PRIOR_INCIDENT_V3_AGENT_RUN_ID = "ecommerce.campaign_planner.A02.real-pilot.v3"
PRIOR_INCIDENT_V3_ATTEMPT_ID = "ecommerce.campaign_planner.A02.real-pilot.v3.attempt-1"
PRIOR_INCIDENT_V2_AGENT_RUN_ID = "ecommerce.campaign_planner.A02.real-pilot.v2"
PRIOR_INCIDENT_V2_ATTEMPT_ID = "ecommerce.campaign_planner.A02.real-pilot.v2.attempt-1"
EARLIER_INCIDENT_AGENT_RUN_ID = "ecommerce.campaign_planner.A02.real-pilot.v1"
EARLIER_INCIDENT_ATTEMPT_ID = "ecommerce.campaign_planner.A02.real-pilot.attempt-1"

QUERY = (
    "请基于以下无敏感信息的内部演示目标给出三条结构化电商增长建议："
    "提升复购；不得调用工具、执行发布或接触客户数据。仅输出简洁建议。"
)
SYSTEM_PROMPT = "你是受限的活动策划师，只给出活动策划方案建议，不执行任何外部动作。"
...
def _task_chain(authority: PilotAuthority):
    ...
    task = tasks.create_task(
        ...
        CreateTaskRequest(
            ...
            title="R2-4J A02 数据参谋 v6 单次真实 Provider 验收",
            description=(
    ...
    if plan_row is None:
        plan = tasks.create_plan(
            ...
            CreatePlanRevisionRequest(
                ...
                steps=[PlanStep(stepKey=STEP_KEY, title="执行并核验 A02 单次真实 Pilot")],
                dependencies=[
                    {
...
def _seal_task(tasks: AipTaskStore, task_run: Any, response: Any) -> Any:
    ...
    for phase, payload in (
        ("think", {"approvalRef": APPROVAL_REF, "decision": "bounded_a02_only"}),
...
def _readback(response: Any, task_run: Any) -> dict[str, Any]:
    ...
    return {
        **build_plan(),
        "status": "R2_A02_REAL_PILOT_GREEN",
...
def main() -> int:
    ...
    return 0 if result.get("status") in {"ready", "R2_A02_REAL_PILOT_GREEN"} else 2
