#!/usr/bin/env python3
"""Execute the single approved D03 real-provider pilot.
...
SYSTEM_PROMPT = "你是受限的数据参谋，只给出建议，不执行任何外部动作。"
    ...
    task = tasks.create_task(
        ...
        CreateTaskRequest(
            ...
            goal={
                ...
                "priorIncidents": build_plan()["priorIncidents"],
            },
        ),
    )
    with db_connect(SCOPE) as conn:
    ...
    if plan_row is None:
        plan = tasks.create_plan(
            ...
            CreatePlanRevisionRequest(
                ...
                steps=[PlanStep(stepKey=STEP_KEY, title="执行并核验 D03 单次真实 Pilot")],
                dependencies=[
                    {
...
def _seal_task(tasks: AipTaskStore, task_run: Any, response: Any) -> Any:
    ...
    for phase, payload in (
        ("think", {"approvalRef": APPROVAL_REF, "decision": "bounded_d03_only"}),
...
def _readback(response: Any, task_run: Any) -> dict[str, Any]:
    ...
    return {
        **build_plan(),
        "status": "R2_D03_REAL_PILOT_GREEN",
...
def main() -> int:
    ...
    return 0 if result.get("status") in {"ready", "R2_D03_REAL_PILOT_GREEN"} else 2
