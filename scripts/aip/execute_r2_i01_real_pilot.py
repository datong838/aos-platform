#!/usr/bin/env python3
"""Execute the single approved I01 real-provider pilot.
...
SKILL_REVISION = 2
SKILL_BINDING_ID = "ecommerce.content_officer.skill.I01.r2"
ROUTE_ID = "route-qyh-image-dev"
CAPABILITY_BINDING_ID = "ecommerce.content_officer.image.generate.r2"
TASK_KEY = "r2-i01-real-pilot-task-v1"
PLAN_KEY = "r2-i01-real-pilot-plan-v1"
TASK_RUN_KEY = "r2-i01-real-pilot-task-run-v1"
AGENT_RUN_ID = "ecommerce.content_officer.I01.real-pilot.v1"
ATTEMPT_ID = "ecommerce.content_officer.I01.real-pilot.v1.attempt-1"
EXECUTE_KEY = "r2-i01-real-pilot-execute-v1"
STEP_KEY = "execute-i01-pilot"

INCIDENT_AGENT_RUN_ID = "ecommerce.content_officer.I01.real-pilot.v7"
INCIDENT_ATTEMPT_ID = "ecommerce.content_officer.I01.real-pilot.v7.attempt-1"
PRIOR_INCIDENT_V6_AGENT_RUN_ID = "ecommerce.content_officer.I01.real-pilot.v6"
PRIOR_INCIDENT_V6_ATTEMPT_ID = "ecommerce.content_officer.I01.real-pilot.v6.attempt-1"
PRIOR_INCIDENT_V5_AGENT_RUN_ID = "ecommerce.content_officer.I01.real-pilot.v5"
PRIOR_INCIDENT_V5_ATTEMPT_ID = "ecommerce.content_officer.I01.real-pilot.v5.attempt-1"
PRIOR_INCIDENT_V4_AGENT_RUN_ID = "ecommerce.content_officer.I01.real-pilot.v4"
PRIOR_INCIDENT_V4_ATTEMPT_ID = "ecommerce.content_officer.I01.real-pilot.v4.attempt-1"
PRIOR_INCIDENT_V3_AGENT_RUN_ID = "ecommerce.content_officer.I01.real-pilot.v3"
PRIOR_INCIDENT_V3_ATTEMPT_ID = "ecommerce.content_officer.I01.real-pilot.v3.attempt-1"
PRIOR_INCIDENT_V2_AGENT_RUN_ID = "ecommerce.content_officer.I01.real-pilot.v2"
PRIOR_INCIDENT_V2_ATTEMPT_ID = "ecommerce.content_officer.I01.real-pilot.v2.attempt-1"
EARLIER_INCIDENT_AGENT_RUN_ID = "ecommerce.content_officer.I01.real-pilot.incident-v0"
EARLIER_INCIDENT_ATTEMPT_ID = "ecommerce.content_officer.I01.real-pilot.incident-v0.attempt-1"

QUERY = (
    "请基于以下无敏感信息的内部演示目标生成一条主图视觉草稿描述："
    "提升复购主题；不得输出真实 URL/二进制；不得调用工具、执行发布或接触客户数据。"
)
SYSTEM_PROMPT = "你是受限的视觉草稿师，只给出视觉草稿方案建议，不执行任何外部动作。"
...
def _task_chain(authority: PilotAuthority):
    ...
    task = tasks.create_task(
        ...
        CreateTaskRequest(
            ...
            title="R2-4J I01 数据参谋 v6 单次真实 Provider 验收",
            description=(
    ...
    if plan_row is None:
        plan = tasks.create_plan(
            ...
            CreatePlanRevisionRequest(
                ...
                steps=[PlanStep(stepKey=STEP_KEY, title="执行并核验 I01 单次真实 Pilot")],
                dependencies=[
                    {
...
def _readback(response: Any, task_run: Any) -> dict[str, Any]:
    ...
    return {
        **build_plan(),
        "status": "R2_I01_REAL_PILOT_GREEN",
...
def main() -> int:
    ...
    return 0 if result.get("status") in {"ready", "R2_I01_REAL_PILOT_GREEN"} else 2
