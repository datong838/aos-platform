#!/usr/bin/env python3
"""Standalone GLM 5.2 .env 3-probe smoke. Not AIP Health and not an AgentRun.

Reads GLM_5_2_KEY / GLM_5_2_URL from a local .env. Optional GLM_5_2_MODEL.
Prints only metadata: never the key, prompt, answer, or response body.
"""
...
ALLOWED_KEYS = ("GLM_5_2_KEY", "GLM_5_2_URL", "GLM_5_2_MODEL")
DEFAULT_ENV = Path("/Users/ddt/work/projects/ai_agent/aos-platform/.env")
DEFAULT_MODEL = "glm-5.2"
DEFAULT_TIMEOUT_MS = 60_000
PROBE_COUNT = 3
PINNED_HOST = "open.bigmodel.cn"
PINNED_BASE_PATH = "/api/coding/paas/v4"
SAFE_PROBES = (
    "请仅回复一个简短的服务可用状态词。",
...
def chat_url(base_url: str) -> str:
    ...
    if parsed.scheme != "https" or parsed.hostname != PINNED_HOST:
        raise SystemExit("blocked: GLM_5_2_URL host is not the pinned BigModel hub")
    path = parsed.path.rstrip("/") or PINNED_BASE_PATH
    if path != PINNED_BASE_PATH:
        raise SystemExit("blocked: GLM_5_2_URL path must be /api/coding/paas/v4")
...
def smoke(*, env_file: Path, timeout_ms: int) -> dict[str, object]:
    ...
    api_key = settings.get("GLM_5_2_KEY") or ""
    base_url = settings.get("GLM_5_2_URL") or ""
    model = settings.get("GLM_5_2_MODEL") or DEFAULT_MODEL
    if not api_key:
        return {
            "status": "blocked",
            "blocker": "missing_glm_5_2_key",
            "envFileExists": env_file.is_file(),
        }
    if not base_url:
        return {"status": "blocked", "blocker": "missing_glm_5_2_url"}
