#!/usr/bin/env python3
"""VID-3: one three-probe Health for agnes-video-qyh-dev (videos/generations).

Metadata only. Never prints Secret, prompt, video URL/id or Provider headers.
"""
...
ACTOR = "aip-r2-video-health-refresh"
APPROVAL_REF = "35-R1-C"
PROVIDER_ID = "agnes-video-qyh-dev"
NETWORK_ID = "network-qyh-video-dev"
PROVIDER_MODEL_ID = "agnes-video-v2.0"
# Domestic Agnes free-tier video RPM is ~1; space distinct probes, never retry a failed one.
PROBE_SPACING_SECONDS = 65

_SAFE_PROBES = (
    "short abstract calm product demo clip no text",
    "minimal geometric motion field no logos",
    "soft gray ambient loop no people",
)
...
def build_plan(
    *, provider_revision: int | None = None, network_revision: int | None = None
    ...
    return {
        ...
        "probeSpacingSeconds": PROBE_SPACING_SECONDS,
    }
...
def refresh(
    *,
    ...
    results = []
    for index, prompt in enumerate(_SAFE_PROBES):
        if index > 0:
    ...
    observation = ProviderHealthObservation(
        ...
        observation_id=(
            f"health-agnes-video-qyh-r2-{observed_at:%Y%m%d%H%M%S%f}"
        ),
        provider=provider_ref,
        status="healthy",
    ...
    store.record_health(
        SCOPE,
        ACTOR,
        f"r2-video-health-{observed_at:%Y%m%d%H%M%S%f}",
        observation,
    )
    return {
        **build_plan(
        ...
        "status": "PROVIDER_VIDEO_HEALTH_REFRESH_GREEN",
...
def main() -> int:
    ...
    return 0 if result.get("status") in {
        "planned",
        "PROVIDER_VIDEO_HEALTH_REFRESH_GREEN",
    } else 2
