"""A02 growth-plan pilot graph and isolated contract-eval authority.
...
A02_GRAPH_ID = "ecommerce.logic.A02"
A02_EVAL_SUITE_ID = "ecommerce.logic.A02.contract.v1"
A02_OUTPUT_CONTRACT = "CampaignPlan.DRAFT"
...
class A02InputGateError(ValueError):
...
class A02ContractEval:
    suite: EvalSuite
    report: EvalReportEvidence
    successful_run: LogicDryRun


def build_a02_graph_request(*, model_id: str) -> CreateLogicGraphRequest:
    """Return the canonical input -> use_llm -> transform A02 graph."""
    ...
    return CreateLogicGraphRequest(
        id=A02_GRAPH_ID,
        name="A02 活动策划方案生成",
        description="基于有来源的非敏感经营摘要生成 CampaignPlan 草稿；禁止外部动作。",
        nodes=[
            LogicGraphNode(
                id="input",
                kind="input",
            ...
            LogicGraphNode(
                ...
                position_x=640,
                position_y=0,
                config={"expression": '"CampaignPlan.DRAFT"'},
            ),
        ],
        edges=[
            LogicGraphEdge(
...
def build_a02_eval_suite() -> EvalSuite:
    ...
    return EvalSuite(
        id=A02_EVAL_SUITE_ID,
        name="A02 活动策划方案生成契约门 v1（隔离 dry-run，非模型效果证据）",
        gate_threshold=1.0,
        cases=[
            TestCase(id="a02-positive", name="结构与 Usage 合约", inputs=safe),
            TestCase(
                id="a02-structure",
                name="拒绝缺失结构字段",
                inputs={
            ...
            TestCase(
                id="a02-fact-boundary",
                name="拒绝无事实来源",
                inputs={**safe, "evidence_refs": []},
            ),
            TestCase(
                id="a02-external-action",
                name="拒绝外部动作",
...
def validate_a02_inputs(inputs: dict[str, Any]) -> None:
    if set(inputs) != _ALLOWED_INPUT_KEYS:
        raise A02InputGateError(
            "A02_INPUT_STRUCTURE_INVALID", "A02 input fields do not match the contract"
        )
    summary = inputs.get("summary")
    evidence_refs = inputs.get("evidence_refs")
    if not isinstance(summary, str) or not summary.strip() or len(summary) > 4_000:
        raise A02InputGateError(
            "A02_INPUT_STRUCTURE_INVALID", "A02 summary is missing or invalid"
        )
    if (
        ...
        raise A02InputGateError(
            "A02_FACT_EVIDENCE_REQUIRED", "A02 facts require non-empty evidence refs"
        )
    if inputs.get("requested_mode") != "draft_only":
        raise A02InputGateError(
            "A02_EXTERNAL_ACTION_DENIED", "A02 only permits draft-only output"
        )
    if inputs.get("data_classification") != "internal_non_sensitive":
        raise A02InputGateError(
            "A02_DATA_CLASSIFICATION_DENIED",
            "A02 only permits approved non-sensitive input",
        )
    if _SENSITIVE_TEXT.search(summary):
        raise A02InputGateError(
            "A02_SENSITIVE_INPUT_DENIED", "A02 input contains secret or PII patterns"
        )


def evaluate_a02_contract(
    graph: LogicGraphSnapshot,
    registry: RuntimeAdapterRegistry,
    *,
    now: datetime | None = None,
) -> A02ContractEval:
    """Evaluate A02 without a Provider call and return persistence-ready evidence."""
    if graph.id != A02_GRAPH_ID:
        raise ValueError("A02 eval target must be the canonical A02 graph")
    suite = build_a02_eval_suite()
    executor = LogicDryRunExecutor(registry)
    results: list[CaseResult] = []
    successful_run: LogicDryRun | None = None
    for case in suite.cases:
        if case.id == "a02-adapter-failure":
            validate_a02_inputs(case.inputs)
            run = executor.execute(
                graph, case.inputs, run_id="logic-run-a02-eval-adapter-failure"
            )
            actual = {
                "status": run.status,
        ...
        elif case.id == "a02-positive":
            validate_a02_inputs(case.inputs)
            run = executor.execute(
                graph, case.inputs, run_id="logic-run-a02-eval-positive"
            )
            ...
            passed = actual == {
                "status": "succeeded",
                "output_contract": A02_OUTPUT_CONTRACT,
                "usage_present": True,
            ...
            expected_code = {
                "a02-structure": "A02_INPUT_STRUCTURE_INVALID",
                "a02-fact-boundary": "A02_FACT_EVIDENCE_REQUIRED",
                "a02-external-action": "A02_EXTERNAL_ACTION_DENIED",
                "a02-sensitive": "A02_SENSITIVE_INPUT_DENIED",
            }[case.id]
            try:
                validate_a02_inputs(case.inputs)
            except A02InputGateError as exc:
    ...
    if successful_run is None:
        raise RuntimeError("A02 positive contract case did not produce a successful run")
    passed_count = sum(result.passed for result in results)
    report = EvalReportEvidence(
        report_id="ecommerce.logic.A02.contract.v1.report",
        suite_id=suite.id,
    ...
    return A02ContractEval(suite=suite, report=report, successful_run=successful_run)
