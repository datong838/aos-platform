"""C02 growth-plan pilot graph and isolated contract-eval authority.
...
C02_GRAPH_ID = "ecommerce.logic.C02"
C02_EVAL_SUITE_ID = "ecommerce.logic.C02.contract.v1"
C02_OUTPUT_CONTRACT = "ContentStrategy.DRAFT"
...
class C02InputGateError(ValueError):
...
class C02ContractEval:
    suite: EvalSuite
    report: EvalReportEvidence
    successful_run: LogicDryRun


def build_c02_graph_request(*, model_id: str) -> CreateLogicGraphRequest:
    """Return the canonical input -> use_llm -> transform C02 graph."""
    ...
    return CreateLogicGraphRequest(
        id=C02_GRAPH_ID,
        name="C02 内容策略生成",
        description="基于有来源的非敏感经营摘要生成 GrowthPlan 草稿；禁止外部动作。",
        nodes=[
            LogicGraphNode(
                id="input",
                kind="input",
            ...
            LogicGraphNode(
                ...
                position_x=640,
                position_y=0,
                config={"expression": '"ContentStrategy.DRAFT"'},
            ),
        ],
        edges=[
            LogicGraphEdge(
...
def build_c02_eval_suite() -> EvalSuite:
    ...
    return EvalSuite(
        id=C02_EVAL_SUITE_ID,
        name="C02 内容策略生成契约门 v1（隔离 dry-run，非模型效果证据）",
        gate_threshold=1.0,
        cases=[
            TestCase(id="d03-positive", name="结构与 Usage 合约", inputs=safe),
            TestCase(
                id="d03-structure",
                name="拒绝缺失结构字段",
                inputs={
            ...
            TestCase(
                id="d03-fact-boundary",
                name="拒绝无事实来源",
                inputs={**safe, "evidence_refs": []},
            ),
            TestCase(
                id="d03-external-action",
                name="拒绝外部动作",
...
def validate_c02_inputs(inputs: dict[str, Any]) -> None:
    if set(inputs) != _ALLOWED_INPUT_KEYS:
        raise C02InputGateError(
            "C02_INPUT_STRUCTURE_INVALID", "C02 input fields do not match the contract"
        )
    summary = inputs.get("summary")
    evidence_refs = inputs.get("evidence_refs")
    if not isinstance(summary, str) or not summary.strip() or len(summary) > 4_000:
        raise C02InputGateError(
            "C02_INPUT_STRUCTURE_INVALID", "C02 summary is missing or invalid"
        )
    if (
        ...
        raise C02InputGateError(
            "C02_FACT_EVIDENCE_REQUIRED", "C02 facts require non-empty evidence refs"
        )
    if inputs.get("requested_mode") != "draft_only":
        raise C02InputGateError(
            "C02_EXTERNAL_ACTION_DENIED", "C02 only permits draft-only output"
        )
    if inputs.get("data_classification") != "internal_non_sensitive":
        raise C02InputGateError(
            "C02_DATA_CLASSIFICATION_DENIED",
            "C02 only permits approved non-sensitive input",
        )
    if _SENSITIVE_TEXT.search(summary):
        raise C02InputGateError(
            "C02_SENSITIVE_INPUT_DENIED", "C02 input contains secret or PII patterns"
        )


def evaluate_c02_contract(
    graph: LogicGraphSnapshot,
    registry: RuntimeAdapterRegistry,
    *,
    now: datetime | None = None,
) -> C02ContractEval:
    """Evaluate C02 without a Provider call and return persistence-ready evidence."""
    if graph.id != C02_GRAPH_ID:
        raise ValueError("C02 eval target must be the canonical C02 graph")
    suite = build_c02_eval_suite()
    executor = LogicDryRunExecutor(registry)
    results: list[CaseResult] = []
    successful_run: LogicDryRun | None = None
    for case in suite.cases:
        if case.id == "d03-adapter-failure":
            validate_c02_inputs(case.inputs)
            run = executor.execute(
                graph, case.inputs, run_id="logic-run-d03-eval-adapter-failure"
            )
            actual = {
                "status": run.status,
        ...
        elif case.id == "d03-positive":
            validate_c02_inputs(case.inputs)
            run = executor.execute(
                graph, case.inputs, run_id="logic-run-d03-eval-positive"
            )
            ...
            passed = actual == {
                "status": "succeeded",
                "output_contract": C02_OUTPUT_CONTRACT,
                "usage_present": True,
            ...
            expected_code = {
                "d03-structure": "C02_INPUT_STRUCTURE_INVALID",
                "d03-fact-boundary": "C02_FACT_EVIDENCE_REQUIRED",
                "d03-external-action": "C02_EXTERNAL_ACTION_DENIED",
                "d03-sensitive": "C02_SENSITIVE_INPUT_DENIED",
            }[case.id]
            try:
                validate_c02_inputs(case.inputs)
            except C02InputGateError as exc:
    ...
    if successful_run is None:
        raise RuntimeError("C02 positive contract case did not produce a successful run")
    passed_count = sum(result.passed for result in results)
    report = EvalReportEvidence(
        report_id="ecommerce.logic.C02.contract.v1.report",
        suite_id=suite.id,
    ...
    return C02ContractEval(suite=suite, report=report, successful_run=successful_run)
