"""D03 growth-plan pilot graph and isolated contract-eval authority.
...
D03_EVAL_SUITE_ID = "ecommerce.logic.D03.contract.v1"
D03_OUTPUT_CONTRACT = "GrowthPlan.DRAFT"
...
class D03InputGateError(ValueError):
...
class D03ContractEval:
    suite: EvalSuite
    report: EvalReportEvidence
    successful_run: LogicDryRun


def build_d03_graph_request(*, model_id: str) -> CreateLogicGraphRequest:
    """Return the canonical input -> use_llm -> transform D03 graph."""
    ...
    return CreateLogicGraphRequest(
        ...
        nodes=[
            ...
            LogicGraphNode(
                ...
                position_x=640,
                position_y=0,
                config={"expression": '"GrowthPlan.DRAFT"'},
            ),
        ],
        edges=[
            LogicGraphEdge(
...
def build_d03_eval_suite() -> EvalSuite:
    ...
    return EvalSuite(
        id=D03_EVAL_SUITE_ID,
        name="D03 增长方案生成契约门 v1（隔离 dry-run，非模型效果证据）",
        gate_threshold=1.0,
        cases=[
...
def validate_d03_inputs(inputs: dict[str, Any]) -> None:
    if set(inputs) != _ALLOWED_INPUT_KEYS:
        raise D03InputGateError(
            "D03_INPUT_STRUCTURE_INVALID", "D03 input fields do not match the contract"
        )
    summary = inputs.get("summary")
    evidence_refs = inputs.get("evidence_refs")
    if not isinstance(summary, str) or not summary.strip() or len(summary) > 4_000:
        raise D03InputGateError(
            "D03_INPUT_STRUCTURE_INVALID", "D03 summary is missing or invalid"
        )
    if (
        ...
        raise D03InputGateError(
            "D03_FACT_EVIDENCE_REQUIRED", "D03 facts require non-empty evidence refs"
        )
    if inputs.get("requested_mode") != "draft_only":
        raise D03InputGateError(
            "D03_EXTERNAL_ACTION_DENIED", "D03 only permits draft-only output"
        )
    if inputs.get("data_classification") != "internal_non_sensitive":
        raise D03InputGateError(
            "D03_DATA_CLASSIFICATION_DENIED",
            "D03 only permits approved non-sensitive input",
        )
    if _SENSITIVE_TEXT.search(summary):
        raise D03InputGateError(
            "D03_SENSITIVE_INPUT_DENIED", "D03 input contains secret or PII patterns"
        )


def evaluate_d03_contract(
    graph: LogicGraphSnapshot,
    registry: RuntimeAdapterRegistry,
    *,
    now: datetime | None = None,
) -> D03ContractEval:
    """Evaluate D03 without a Provider call and return persistence-ready evidence."""
    if graph.id != D03_GRAPH_ID:
        raise ValueError("D03 eval target must be the canonical D03 graph")
    suite = build_d03_eval_suite()
    ...
    for case in suite.cases:
        if case.id == "d03-adapter-failure":
            ...
            passed = actual == {
                "status": "succeeded",
                "output_contract": D03_OUTPUT_CONTRACT,
                "usage_present": True,
            ...
            expected_code = {
                "d03-structure": "D03_INPUT_STRUCTURE_INVALID",
                "d03-fact-boundary": "D03_FACT_EVIDENCE_REQUIRED",
                "d03-external-action": "D03_EXTERNAL_ACTION_DENIED",
                "d03-sensitive": "D03_SENSITIVE_INPUT_DENIED",
            }[case.id]
            try:
                validate_d03_inputs(case.inputs)
            except D03InputGateError as exc:
    ...
    if successful_run is None:
        raise RuntimeError("D03 positive contract case did not produce a successful run")
    passed_count = sum(result.passed for result in results)
    report = EvalReportEvidence(
        report_id="ecommerce.logic.D03.contract.v1.report",
        suite_id=suite.id,
    ...
    return D03ContractEval(suite=suite, report=report, successful_run=successful_run)
