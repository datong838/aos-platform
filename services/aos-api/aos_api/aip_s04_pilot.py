"""S04 growth-plan pilot graph and isolated contract-eval authority.
...
S04_GRAPH_ID = "ecommerce.logic.S04"
S04_EVAL_SUITE_ID = "ecommerce.logic.S04.contract.v1"
S04_OUTPUT_CONTRACT = "ServiceAction.DRAFT"
...
class S04InputGateError(ValueError):
...
class S04ContractEval:
    suite: EvalSuite
    report: EvalReportEvidence
    successful_run: LogicDryRun


def build_s04_graph_request(*, model_id: str) -> CreateLogicGraphRequest:
    """Return the canonical input -> use_llm -> transform S04 graph."""
    ...
    return CreateLogicGraphRequest(
        id=S04_GRAPH_ID,
        name="S04 售后策略生成",
        description="基于有来源的非敏感经营摘要生成 ServiceAction 草稿；禁止外部动作。",
        nodes=[
            LogicGraphNode(
                id="input",
                kind="input",
            ...
            LogicGraphNode(
                ...
                position_x=640,
                position_y=0,
                config={"expression": '"ServiceAction.DRAFT"'},
            ),
        ],
        edges=[
            LogicGraphEdge(
...
def build_s04_eval_suite() -> EvalSuite:
    ...
    return EvalSuite(
        id=S04_EVAL_SUITE_ID,
        name="S04 售后策略生成契约门 v1（隔离 dry-run，非模型效果证据）",
        gate_threshold=1.0,
        cases=[
            TestCase(id="s04-positive", name="结构与 Usage 合约", inputs=safe),
            TestCase(
                id="s04-structure",
                name="拒绝缺失结构字段",
                inputs={
            ...
            TestCase(
                id="s04-fact-boundary",
                name="拒绝无事实来源",
                inputs={**safe, "evidence_refs": []},
            ),
            TestCase(
                id="s04-external-action",
                name="拒绝外部动作",
...
def validate_s04_inputs(inputs: dict[str, Any]) -> None:
    if set(inputs) != _ALLOWED_INPUT_KEYS:
        raise S04InputGateError(
            "S04_INPUT_STRUCTURE_INVALID", "S04 input fields do not match the contract"
        )
    summary = inputs.get("summary")
    evidence_refs = inputs.get("evidence_refs")
    if not isinstance(summary, str) or not summary.strip() or len(summary) > 4_000:
        raise S04InputGateError(
            "S04_INPUT_STRUCTURE_INVALID", "S04 summary is missing or invalid"
        )
    if (
        ...
        raise S04InputGateError(
            "S04_FACT_EVIDENCE_REQUIRED", "S04 facts require non-empty evidence refs"
        )
    if inputs.get("requested_mode") != "draft_only":
        raise S04InputGateError(
            "S04_EXTERNAL_ACTION_DENIED", "S04 only permits draft-only output"
        )
    if inputs.get("data_classification") != "internal_non_sensitive":
        raise S04InputGateError(
            "S04_DATA_CLASSIFICATION_DENIED",
            "S04 only permits approved non-sensitive input",
        )
    if _SENSITIVE_TEXT.search(summary):
        raise S04InputGateError(
            "S04_SENSITIVE_INPUT_DENIED", "S04 input contains secret or PII patterns"
        )


def evaluate_s04_contract(
    graph: LogicGraphSnapshot,
    registry: RuntimeAdapterRegistry,
    *,
    now: datetime | None = None,
) -> S04ContractEval:
    """Evaluate S04 without a Provider call and return persistence-ready evidence."""
    if graph.id != S04_GRAPH_ID:
        raise ValueError("S04 eval target must be the canonical S04 graph")
    suite = build_s04_eval_suite()
    executor = LogicDryRunExecutor(registry)
    results: list[CaseResult] = []
    successful_run: LogicDryRun | None = None
    for case in suite.cases:
        if case.id == "s04-adapter-failure":
            validate_s04_inputs(case.inputs)
            run = executor.execute(
                graph, case.inputs, run_id="logic-run-s04-eval-adapter-failure"
            )
            actual = {
                "status": run.status,
        ...
        elif case.id == "s04-positive":
            validate_s04_inputs(case.inputs)
            run = executor.execute(
                graph, case.inputs, run_id="logic-run-s04-eval-positive"
            )
            ...
            passed = actual == {
                "status": "succeeded",
                "output_contract": S04_OUTPUT_CONTRACT,
                "usage_present": True,
            ...
            expected_code = {
                "s04-structure": "S04_INPUT_STRUCTURE_INVALID",
                "s04-fact-boundary": "S04_FACT_EVIDENCE_REQUIRED",
                "s04-external-action": "S04_EXTERNAL_ACTION_DENIED",
                "s04-sensitive": "S04_SENSITIVE_INPUT_DENIED",
            }[case.id]
            try:
                validate_s04_inputs(case.inputs)
            except S04InputGateError as exc:
    ...
    if successful_run is None:
        raise RuntimeError("S04 positive contract case did not produce a successful run")
    passed_count = sum(result.passed for result in results)
    report = EvalReportEvidence(
        report_id="ecommerce.logic.S04.contract.v1.report",
        suite_id=suite.id,
    ...
    return S04ContractEval(suite=suite, report=report, successful_run=successful_run)
