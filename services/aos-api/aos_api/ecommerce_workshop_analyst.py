...
from aos_api.ecommerce_workshop_analyst_contracts import ANALYST_SCHEMA_VERSION, ANALYST_SCENARIO_SCHEMA_VERSION, AnalystAxisReadiness, AnalystBlocker, AnalystCountLedger, AnalystPageInfo, AnalystReadinessAxis, AnalystViewId, AnalystViewSlice, WorkshopAnalystViewEnvelope
...
class EcommerceWorkshopAnalyst:
    ...
    def __init__(self, *, reader: AnalystCanonicalReader | None = None, growth_scenario: EcommerceWorkshopGrowthScenario | None = None, clock: Clock | None = None) -> None:
        self._reader = reader
        self._growth_scenario = growth_scenario
    ...
    def read(self, *, org_id: str, project_id: str) -> WorkshopAnalystViewEnvelope:
        ...
        if len(trusted_revisions) > 1:
            ...
            scenario = self._growth_scenario.read(scope=scope, cutoff=cutoff) if self._growth_scenario is not None else None
            return WorkshopAnalystViewEnvelope(schemaVersion=ANALYST_SCENARIO_SCHEMA_VERSION if scenario is not None else ANALYST_SCHEMA_VERSION, tenant=TenantContext(org_id=org_id, project_id=project_id), resource_revision=1, evaluated_at=cutoff, data_cutoff=cutoff, views=blocked_views, growthScenario=scenario, page=AnalystPageInfo(count=0))
        ...
        scenario = self._growth_scenario.read(scope=scope, cutoff=cutoff) if self._growth_scenario is not None else None
        return WorkshopAnalystViewEnvelope(schemaVersion=ANALYST_SCENARIO_SCHEMA_VERSION if scenario is not None else ANALYST_SCHEMA_VERSION, tenant=TenantContext(org_id=org_id, project_id=project_id), resource_revision=final_revision, evaluated_at=cutoff, data_cutoff=cutoff, views=normalized, growthScenario=scenario, page=AnalystPageInfo(count=sum(len(item.metrics) for item in normalized)))
