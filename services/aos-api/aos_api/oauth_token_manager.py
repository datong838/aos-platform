...
class OAuthTokenManager:
    def __init__(
        ...
        *,
        ...
        self.store, self.transport, self.audit = (
            store,
            transport,
            audit or (lambda *_: None),
        )
        self.clock, self.refresh_skew = (
    ...
    def _emit(
        self, event: str, scope: OAuthScope, fields: dict[str, Any], started: float
    ) -> None:
        self.audit(
            event,
            scope,
        ...
        *,
        ...
        if response.status >= 400:
            self._emit(
                ...
                scope,
                {"status": "failed", "errorCode": self._error_code(response)},
                started,
            )
            raise OAuthError(self._error_code(response))
        try:
            record = self._record(
                scope, token_endpoint, client_id, client_secret, response.data
            )
        except (KeyError, TypeError, ValueError, OAuthError) as exc:
            self._emit(
                "oauth.exchange.failure",
                scope,
                {"status": "failed", "errorCode": "OAUTH_RESPONSE_INVALID"},
                started,
            )
        ...
        self._emit(
            ...
            scope,
            {"tokenId": record.token_id, "status": record.status},
            started,
        )
        return record
    ...
    def get_valid_access_token(self, scope: OAuthScope) -> str:
        ...
        if (
            ...
            or record.expires_at > self.clock() + self.refresh_skew
        ):
        ...
        with self._lock(scope):
            ...
            if (
                ...
                or current.expires_at > self.clock() + self.refresh_skew
            ):
                return self._decrypt(current, "access_token")
            return self._refresh(current)

    def refresh_due(self, tenant_scope: TenantScope) -> list[str]:
        refreshed: list[str] = []
        for record in self.store.list_due(
            tenant_scope, self.clock() + self.refresh_skew
        ):
            try:
    ...
    def revoke(self, scope: OAuthScope) -> None:
        ...
        self._emit(
            ...
            scope,
            {"tokenId": record.token_id, "status": "revoked"},
            started,
        )
    ...
    def _refresh(self, record: OAuthTokenRecord) -> str:
        started = time.monotonic()
        refresh_token = (
            self._decrypt(record, "refresh_token")
        ...
        form = {
            ...
            "client_secret": self._decrypt(record, "client_secret")
            if record.encrypted_client_secret
            else "",
        }
        ...
        if response.status >= 400:
            ...
            self._emit(
                ...
                record.scope,
                {"tokenId": record.token_id, "status": "failed", "errorCode": code},
                started,
            )
            raise OAuthError("OAUTH_REFRESH_FAILED")
        data = response.data
        record.encrypted_access_token = encrypt_strict(
            str(data["access_token"]),
            aad=self._aad(record.scope, record.token_id, "access_token"),
        )
        if data.get("refresh_token"):
            record.encrypted_refresh_token = encrypt_strict(
                str(data["refresh_token"]),
                aad=self._aad(record.scope, record.token_id, "refresh_token"),
            )
        record.expires_at = (
            self.clock() + timedelta(seconds=max(0, int(data.get("expires_in") or 0)))
        ...
        self._emit(
            ...
            record.scope,
            {"tokenId": record.token_id, "status": "active"},
            started,
        )
    ...
    def _record(
        ...
        record = OAuthTokenRecord(
            ...
            encrypted_access_token=encrypt_strict(
                str(data["access_token"]),
                aad=self._aad(scope, token_id, "access_token"),
            ),
            client_id=client_id,
            expires_at=expires,
        ...
        if data.get("refresh_token"):
            record.encrypted_refresh_token = encrypt_strict(
                str(data["refresh_token"]),
                aad=self._aad(scope, token_id, "refresh_token"),
            )
        if client_secret:
            record.encrypted_client_secret = encrypt_strict(
                client_secret, aad=self._aad(scope, token_id, "client_secret")
            )
        return record

    def _decrypt(self, record: OAuthTokenRecord, field: str) -> str:
        value = getattr(record, "encrypted_" + field)
        return decrypt_strict(
            value, aad=self._aad(record.scope, record.token_id, field)
        )
    ...
    def _call(
        self, endpoint: str, form: dict[str, str], timeout: float
    ) -> OAuthResponse:
