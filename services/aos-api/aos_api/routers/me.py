...
def _workspace_name(org_id: str, project_id: str) -> str:
    """TWA.2 · 产品名；技术 id 仍为 project_id。"""
    from aos_api.orgs import workspace_display_name
    return workspace_display_name(org_id, project_id)
...
def me(principal: Principal = Depends(require_principal)) -> dict:
    ...
    ws = _workspace_name(principal.org_id, principal.project_id)
