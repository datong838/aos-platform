...
def create_schedule(
    req: CreateScheduleRequest, principal: PrincipalDep
):
    sched = Schedule(
    ...
    try:
        return get_engine().create_schedule(_scope(principal), sched)
...
def list_schedules(
    ...
    return {
        "items": get_engine().list_schedules(
            _scope(principal), enabled_only=enabled_only
        )
    }
...
def get_schedule(
    sched_id: str, principal: PrincipalDep
):
    sched = get_engine().get_schedule(_scope(principal), sched_id)
    if sched is None:
...
def delete_schedule(
    ...
    ok = get_engine().delete_schedule(_scope(principal), sched_id)
    if not ok:
        raise ApiError(code="NOT_FOUND", message=f"调度 {sched_id} 不存在", status_code=404)
...
def get_next_run(
    ...
    try:
        nr = get_engine().get_next_run(_scope(principal), sched_id)
...
def trigger_schedule(
    ...
    try:
        return get_engine().trigger(_scope(principal), sched_id)
    except SchedulingError as err:
        raise _map_error(err) from err
...
def assign_resource(
    ...
    return get_engine().assign_resource(_scope(principal), res)
...
def get_resources(
    ...
    return {"items": get_engine().get_resources(_scope(principal), sched_id)}
...
def get_history(
    ...
    return {"items": get_engine().history(_scope(principal), sched_id)}
