#!/usr/bin/env python3
"""Operate the TI-2 E3 Module identity backfill state machine."""
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--code-commit", default="472728e")
...
def main() -> int:
    ...
    actor_hash = stable_key_hash("TI-2-E3-ACTOR", args.actor)
        ...
        if args.action in {"plan", "persist"}:
            ...
            if args.action == "persist":
                result["persistence"] = persist_plan(
                    conn,
                    plan=plan,
                    environment_hash=stable_key_hash(
                        "TI-2-E3-ENV", args.environment
                    ),
