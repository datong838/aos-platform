...
class Conn:
    ...
    def execute(self,query,args):
        ...
        return Result({**self.row,"status":"running","version":2} if self.updated else None)
    def commit(self):
...
def test_default_capacity_gate_keeps_real_start_blocked():
    service=AipAgentRunService(factory(Conn(row())),model_resolver=Resolver(resolution()))
