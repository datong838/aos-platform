...
def test_exact_adapter_returns_authority_refs_without_secret_or_mock():
    adapter=LLMAdapter(resolver=Resolver(resolution()),provider_invoker=lambda resolved,prompt:{"answer":"真实回答","provider":"provider-1","model":"model-1","tokens":12,"route":"exact"})
    result=adapter.chat_exact(SCOPE,"route-1","问题")
...
def test_legacy_and_blocked_calls_fail_closed():
    ...
    with pytest.raises(LLMRuntimeBlocked,match="provider_health_unavailable_or_stale"):adapter.chat_exact(SCOPE,"route-1","问题")
...
def test_mock_or_unknown_usage_is_rejected(response):
    adapter=LLMAdapter(resolver=Resolver(resolution()),provider_invoker=lambda *_:response)
    with pytest.raises(LLMRuntimeBlocked):adapter.chat_exact(SCOPE,"route-1","问题")
