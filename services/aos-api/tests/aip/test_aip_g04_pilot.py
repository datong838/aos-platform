...
def _snapshot() -> LogicGraphSnapshot:
    request = build_g04_graph_request(model_id=MODEL_ID)
...
def test_g04_graph_is_exact_three_node_dag() -> None:
    request = build_g04_graph_request(model_id=MODEL_ID)
    assert request.id == G04_GRAPH_ID
...
def test_g04_contract_eval_passes_six_cases_without_production_write() -> None:
    calls: list[str] = []
    result = evaluate_g04_contract(
        _snapshot(),
        _registry(calls),
...
def test_g04_sensitive_input_is_rejected_before_adapter_boundary() -> None:
    # Case ids use g04-* for this Skill revision.
    # production Skill G04 r2 EvalSuite was published with these ids.
    case = next(
        case for case in build_g04_eval_suite().cases if case.id == "g04-sensitive"
    )
    with pytest.raises(G04InputGateError) as exc_info:
        validate_g04_inputs(case.inputs)
    assert exc_info.value.code == "G04_SENSITIVE_INPUT_DENIED"


@pytest.mark.parametrize("model_id", ["", "   "])
def test_g04_graph_requires_exact_model_id(model_id: str) -> None:
    with pytest.raises(ValueError, match="model_id is required"):
        build_g04_graph_request(model_id=model_id)
