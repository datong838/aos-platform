...
def _snapshot() -> LogicGraphSnapshot:
    request = build_p02_graph_request(model_id=MODEL_ID)
...
def test_p02_graph_is_exact_three_node_dag() -> None:
    request = build_p02_graph_request(model_id=MODEL_ID)
    assert request.id == P02_GRAPH_ID
...
def test_p02_contract_eval_passes_six_cases_without_production_write() -> None:
    calls: list[str] = []
    result = evaluate_p02_contract(
        _snapshot(),
        _registry(calls),
...
def test_p02_sensitive_input_is_rejected_before_adapter_boundary() -> None:
    # Case ids use p02-* for this Skill revision.
    # production Skill P02 r2 EvalSuite was published with these ids.
    case = next(
        case for case in build_p02_eval_suite().cases if case.id == "p02-sensitive"
    )
    with pytest.raises(P02InputGateError) as exc_info:
        validate_p02_inputs(case.inputs)
    assert exc_info.value.code == "P02_SENSITIVE_INPUT_DENIED"


@pytest.mark.parametrize("model_id", ["", "   "])
def test_p02_graph_requires_exact_model_id(model_id: str) -> None:
    with pytest.raises(ValueError, match="model_id is required"):
        build_p02_graph_request(model_id=model_id)
