...
def assembly_image(*, scope=TenantScope("org-org", "dev-project")):
    host = "api.agnes-ai.cn"
    region = "China (Domestic)"
    provider_ref = VersionedAssetRef(
        assetType="ProviderInstanceRevision",
        assetId="agnes-image-qyh-dev",
        revision=2,
        contentHash=HASH,
    )
    egress_ref = ref("EgressPolicyRevision", "agnes-image-qyh-dev-egress")
    data_ref = ref(
        "DataClassificationPolicyRevision", "agnes-text-qyh-dev-data-classification"
    )
    network_ref = ref("NetworkPolicyRevision", "network-qyh-image-dev")
    provider = SimpleNamespace(
        tenant=SimpleNamespace(org_id=scope.org_id, project_id=scope.project_id),
        provider_instance_id=provider_ref.asset_id,
        revision=2,
        ...
        plugin_ref=ref("ProviderPluginRevision", "agnes-image"),
        endpoint_profile=SimpleNamespace(
    ...
    assets = SimpleNamespace(
        ...
        plugin=SimpleNamespace(
            modalities=["image"],
            approved_capabilities=["image"],
            default_models=["agnes-image-2.1-flash"],
        ),
    )
    request = R1BootstrapProbeRequest(
        provider=provider_ref,
...
def test_image_probe_uses_generations_endpoint_and_keeps_no_payload():
    assets, request = assembly_image()
    transport = Transport(
    ...
    assert transport.calls[0]["url"] == "https://api.agnes-ai.cn/v1/images/generations"
    assert "example.invalid" not in repr(result)
    assert "top-secret" not in repr(result)


def test_image_probe_fails_closed_without_image_payload():
    assets, request = assembly_image()
    transport = Transport(
        ProviderTransportResponse(
            status_code=200,
            payload={"model": "agnes-image-2.1-flash", "data": [{}]},
        )
    )
    with pytest.raises(
        R1BootstrapProbeBlocked, match="provider_response_answer_missing"
    ):
