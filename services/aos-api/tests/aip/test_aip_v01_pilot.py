...
MODEL_ID = "agnes-video-v2.0@exact-r1"
...
def test_v01_graph_is_exact_three_node_dag() -> None:
    request = build_i01_graph_request(model_id=MODEL_ID)
    assert request.id == V01_GRAPH_ID
...
def test_v01_contract_eval_passes_six_cases_without_production_write() -> None:
    calls: list[str] = []
...
def test_v01_sensitive_input_is_rejected_before_adapter_boundary() -> None:
    case = next(
        case for case in build_i01_eval_suite().cases if case.id == "v01-sensitive"
    )
    with pytest.raises(V01InputGateError) as exc_info:
        validate_i01_inputs(case.inputs)
    assert exc_info.value.code == "V01_SENSITIVE_INPUT_DENIED"


@pytest.mark.parametrize("model_id", ["", "   "])
def test_v01_graph_requires_exact_model_id(model_id: str) -> None:
