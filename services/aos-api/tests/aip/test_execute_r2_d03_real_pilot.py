...
def test_r2_4z_uses_v8_authority_and_preserves_v1_to_v7_incident_chain() -> None:
    plan = MODULE.build_plan()
    assert MODULE.APPROVAL_REF == "46-R2-4Z-V8-LINEAGE-BEFORE-USAGE"
    assert MODULE.AGENT_RUN_ID.endswith("real-pilot.v8")
    assert MODULE.ATTEMPT_ID.endswith("real-pilot.v8.attempt-1")
    assert MODULE.EXECUTE_KEY.endswith("execute-v8")
    assert MODULE.INCIDENT_AGENT_RUN_ID.endswith("real-pilot.v7")
    assert MODULE.PRIOR_INCIDENT_V6_AGENT_RUN_ID.endswith("real-pilot.v6")
    assert MODULE.PRIOR_INCIDENT_V4_AGENT_RUN_ID.endswith("real-pilot.v4")
    assert MODULE.PRIOR_INCIDENT_V3_AGENT_RUN_ID.endswith("real-pilot.v3")
    ...
    assert len(plan["priorIncidents"]) == 7
    assert plan["priorIncidents"][-1] == plan["recoveryOf"]
