...
SCRIPT = (
    ...
    / "scripts"
    / "aip"
    / "bootstrap_r08_shopping_advisor_skill_authority.py"
)
SPEC = importlib.util.spec_from_file_location(
    "bootstrap_r08_shopping_advisor_skill_authority", SCRIPT
)
...
def test_plan_is_exact_and_side_effect_free() -> None:
    ...
    assert plan["preservedAuthority"] == ["ecommerce.skill.G04@r2"]
    ...
    assert "recommendation delivery" in plan["forbiddenSideEffects"]


def test_expected_results_are_independent_and_complete() -> None:
    expected_counts = {"G01": 6, "G02": 10, "G03": 10, "G05": 10, "G06": 11}
    for logic_id in module.SHOPPING_ADVISOR_LOGIC_IDS:
    ...
    assert module._expected_results("G02")["g02-inventory-stale"]["code"] == "G02_INVENTORY_STALE"
    assert module._expected_results("G06")["g06-outcome-guessed"]["code"] == "G06_OUTCOME_SOURCE_REQUIRED"
