...
def test_d4_workspace_fk_precheck_has_no_non_null_orphans() -> None:
...
def test_d4_validate_downgrade_upgrade_preserves_rows_and_unknowns() -> None:
    cfg = _config()
    command.downgrade(cfg, "228ti4d1expand")
    with connect() as conn:
        before = _counts(conn)
    command.upgrade(cfg, "228ti4d4validate")
    with connect() as conn:
        assert _validated(conn) == {
            f"fk_{table}_workspace_ti4d1": True for table in TABLES
        }
        assert _counts(conn) == before
    command.downgrade(cfg, "228ti4d1expand")
    with connect() as conn:
        assert _validated(conn) == {
            f"fk_{table}_workspace_ti4d1": False for table in TABLES
        }
        assert _counts(conn) == before
    command.upgrade(cfg, "228ti4d4validate")
    with connect() as conn:
