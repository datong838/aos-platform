...
from aos_api.oauth_token_store import (
...
def test_explicit_refresh_token_grant_is_supported():
    ...
    assert (
        captured["grant_type"] == "refresh_token"
        and captured["refresh_token"] == "old-refresh"
    )
...
def test_lazy_refresh_rotation_and_single_flight():
    ...
    def transport(_url, form, _timeout):
        ...
        if form["grant_type"] == "authorization_code":
            return OAuthResponse(
                200, {"access_token": "old", "refresh_token": "r1", "expires_in": 1}
            )
        return OAuthResponse(
    ...
    store = InMemoryOAuthTokenStore()
    manager = oauth_manager(
        store, transport, clock=lambda: now, refresh_skew=timedelta(minutes=5)
    )
    manager.exchange(
        scope(),
        token_endpoint="https://oauth.example/token",
    ...
    threads = [
        threading.Thread(
            target=lambda: values.append(manager.get_valid_access_token(scope()))
        )
        for _ in range(5)
    ]
...
def test_invalid_grant_requires_reauth_and_revoke_blocks_use():
    ...
    responses = [
        OAuthResponse(
            200, {"access_token": "a", "refresh_token": "r", "expires_in": 0}
        ),
        OAuthResponse(400, {"error": "invalid_grant"}),
    ]
    manager = oauth_manager(
        store, lambda *_: responses.pop(0), refresh_skew=timedelta(minutes=5)
    )
    manager.exchange(
        scope(),
        token_endpoint="https://oauth.example/token",
    ...
    manager2 = oauth_manager(
        InMemoryOAuthTokenStore(), lambda *_: OAuthResponse(200, {"access_token": "a"})
    )
    manager2.exchange(
        scope(),
        token_endpoint="https://oauth.example/token",
    ...
    with pytest.raises(OAuthError):
        manager2.get_valid_access_token(scope())
...
def test_audit_contains_no_secrets():
    ...
    manager = oauth_manager(
        ...
        lambda *_: OAuthResponse(
            200,
            {"access_token": "sensitive-access", "refresh_token": "sensitive-refresh"},
        ),
    ...
    manager.exchange(
        ...
        token_endpoint="https://oauth.example/token",
        client_id="id",
        client_secret="sensitive-client",
        grant_type="client_credentials",
    )
...
def test_refresh_retries_429_and_5xx():
    ...
    replies = [
        OAuthResponse(
            200, {"access_token": "old", "refresh_token": "r", "expires_in": 0}
        ),
...
def test_oauth_redirect_target_is_revalidated():
    manager = oauth_manager(
        InMemoryOAuthTokenStore(),
        lambda *_: OAuthResponse(
            200, {"access_token": "secret"}, final_url="https://127.0.0.1/token"
        ),
    )
    with pytest.raises(OAuthError, match="OAUTH_ENDPOINT_FORBIDDEN"):
        manager.exchange(
...
def test_refresh_revalidates_stored_token_endpoint():
    ...
    def transport(*_):
        ...
        calls += 1
        return OAuthResponse(
            200, {"access_token": "old", "refresh_token": "refresh", "expires_in": 0}
        )
...
def test_in_memory_store_returns_detached_records_and_conflict_does_not_pollute():
    ...
    manager = oauth_manager(
        store,
        lambda *_: OAuthResponse(200, {"access_token": "access", "expires_in": 3600}),
    )
    manager.exchange(
        scope(),
        token_endpoint="https://oauth.example.test/token",
    ...
    assert (
        ...
        and store.get(scope()).status == "reauth_required"
    )
...
def test_postgres_upsert_returns_payload_with_database_version(monkeypatch):
    ...
    original = OAuthTokenRecord(
        "token", scope(), "https://oauth.example.test/token", "enc:v2:value"
    )
