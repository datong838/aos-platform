...
def engine_with_data() -> SchedulingEngine:
    ...
    s1 = eng.create_schedule(SCOPE, Schedule(
    ...
    eng.create_schedule(SCOPE, Schedule(
...
def test_build_view_returns_bars(engine_with_data: SchedulingEngine):
    ...
    view = gantt.build_view(SCOPE, scope="all", horizon_hours=24)
...
def test_build_view_scope_filter(engine_with_data: SchedulingEngine):
    gantt = GanttEngine(engine_with_data)
    project_view = gantt.build_view(SCOPE, scope="project", horizon_hours=24)
    org_view = gantt.build_view(SCOPE, scope="org", horizon_hours=24)
...
def test_invalid_horizon_raises(engine_with_data: SchedulingEngine):
    ...
    with pytest.raises(GanttError) as exc:
        gantt.build_view(SCOPE, horizon_hours=99999)
    assert exc.value.code == "INVALID_HORIZON"


def test_build_for_schedule(engine_with_data: SchedulingEngine):
    gantt = GanttEngine(engine_with_data)
    view = gantt.build_for_schedule(SCOPE, "sch-1", horizon_hours=48)
    assert view.total_bars > 0
...
def test_build_for_schedule_unknown_raises(engine_with_data: SchedulingEngine):
    gantt = GanttEngine(engine_with_data)
    with pytest.raises(GanttError) as exc:
        gantt.build_for_schedule(SCOPE, "ghost")
...
def test_resource_overlap_violation():
    eng = SchedulingEngine(executor=lambda s: None)
    s1 = eng.create_schedule(SCOPE, Schedule(id="s1", name="任务A", cron="0 * * * *"))
    s2 = eng.create_schedule(SCOPE, Schedule(id="s2", name="任务B", cron="0 * * * *"))
    for sid in ("s1", "s2"):
        eng.assign_resource(SCOPE, ScheduledResource(
    ...
    view = gantt.build_view(SCOPE, scope="all", horizon_hours=2)
...
def test_disabled_schedule_violation():
    ...
    gantt = GanttEngine(eng)
    view = gantt.build_view(SCOPE, scope="all", horizon_hours=2, include_disabled=True)
...
def test_overtime_violation():
    eng = SchedulingEngine(executor=lambda s: None)
    eng.create_schedule(SCOPE, Schedule(id="s1", name="任务", cron="0 * * * *"))
    from aos_api.scheduling_engine import ScheduleExecution
    ...
    gantt = GanttEngine(eng)
    view = gantt.build_view(SCOPE, scope="all", horizon_hours=1, duration_minutes=60)
...
def test_no_violations_for_non_overlapping():
    eng = SchedulingEngine(executor=lambda s: None)
    eng.create_schedule(SCOPE, Schedule(id="s1", name="任务", cron="0 0 1 1 *"))
    gantt = GanttEngine(eng)
    view = gantt.build_view(SCOPE, scope="all", horizon_hours=2)
