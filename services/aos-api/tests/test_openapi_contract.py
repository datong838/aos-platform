...
def test_committed_artifacts_are_canonical_and_structurally_valid() -> None:
    ...
    assert len(schema["paths"]) == 2242
    assert len(schema.get("components", {}).get("schemas", {})) == 1399
...
def test_inventory_preserves_route_rows_and_known_duplicates() -> None:
    ...
    assert summary["routeRows"] == 3992
    assert summary["uniqueOperationPairs"] == 3973
    ...
    assert len(inventory["routes"]) == 3992
    assert [row["ordinal"] for row in inventory["routes"]] == list(range(3992))
