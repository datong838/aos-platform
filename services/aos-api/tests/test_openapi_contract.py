...
def test_committed_artifacts_are_canonical_and_structurally_valid() -> None:
    ...
    assert len(schema["paths"]) == 2252
    assert len(schema.get("components", {}).get("schemas", {})) == 1418
...
def test_inventory_preserves_route_rows_and_known_duplicates() -> None:
    ...
    assert summary["routeRows"] == 4011
    assert summary["uniqueOperationPairs"] == 3992
    ...
    assert len(inventory["routes"]) == 4011
    assert [row["ordinal"] for row in inventory["routes"]] == list(range(4011))
