...
def test_committed_artifacts_are_canonical_and_structurally_valid() -> None:
    ...
    assert len(schema["paths"]) == 2284
    assert len(schema.get("components", {}).get("schemas", {})) == 1499
...
def test_inventory_preserves_route_rows_and_known_duplicates() -> None:
    ...
    assert summary["routeRows"] == 4047
    assert summary["uniqueOperationPairs"] == 4028
    ...
    assert len(inventory["routes"]) == 4047
    assert [row["ordinal"] for row in inventory["routes"]] == list(range(4047))
