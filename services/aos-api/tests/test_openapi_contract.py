...
def test_committed_artifacts_are_canonical_and_structurally_valid() -> None:
    ...
    assert len(schema["paths"]) == 2697
    assert len(schema.get("components", {}).get("schemas", {})) == 2524
...
def test_full_video_scenario_is_principal_scoped_and_get_only() -> None:
    schema = json.loads(OPENAPI_PATH.read_bytes())
    path = "/v1/ecommerce-workshop/views/media-studio/full-production-scenario"
    assert set(schema["paths"][path]) == {"get"}
    operation = schema["paths"][path]["get"]
    assert operation["operationId"] == "ecommerceWorkshopMediaStudioFullProductionScenarioGet"
    assert operation["security"] == [{"HTTPBearer": []}]
    assert all(item["in"] != "query" for item in operation.get("parameters", []))
    response_ref = operation["responses"]["200"]["content"]["application/json"]["schema"]["$ref"]
    assert response_ref.endswith("/FullVideoScenarioContribution")
