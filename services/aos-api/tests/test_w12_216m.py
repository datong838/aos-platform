"""216m — jdbc-sqlserver optional ingest."""
...
def _ensure_connector_scope() -> None:
...
def test_216m_mssql_ingest_stub(client):
    client.post("/v1/connector-plugins/jdbc-sqlserver/install", headers=_auth())
    r = client.post(
        "/v1/connectors/jdbc-sqlserver/ingest",
        headers=_auth(),
        json={"limit": 1},
    )
...
def test_216m_mssql_ingest_mock(client, monkeypatch):
    ...
    assert body["pluginId"] == "jdbc-sqlserver"
